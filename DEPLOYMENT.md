...
8. Nếu release có thay đổi Firestore Rules, chạy `npm run deploy:rules` sau khi commit đã được push và lưu bằng chứng CLI thành công. Lệnh này tự chạy `release:predeploy` để xác nhận SHA local/remote trước khi deploy Rules.
   Nếu thay đổi cấu hình indexes, chạy `npm run deploy:indexes` từ cùng commit đã push; đối chiếu index production trước/sau và không chấp nhận xóa composite index ngoài phạm vi. Xem `docs/firestore-index-exemptions.md` để kiểm tra các trường snapshot được miễn index và quy trình rollback.
