function generateCustomReport_chloroform_gcms(templateId, metadata, samples, folder, fileName, version) {
  ...
  const sopConfig = CONFIG.SOP_CONFIG['chloroform-gcms'];
  ...
  if (calibrationTable) {
    Logger.log(`[ChloroformCustom] Tìm thấy bảng đường chuẩn (${numRows} dòng).`);
    // Điền R2
    ...
    Logger.log('[ChloroformCustom] CẢNH BÁO: Không tìm thấy bảng đường chuẩn.');
  }
