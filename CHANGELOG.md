...
- Khôi phục đầy đủ stylesheet, font và icon để giao diện LIMS hiển thị ổn định trên Vercel khi CSP đang bật.
...
- Đóng gói Open Sans, Font Awesome và avatar SVG nội bộ thay vì phụ thuộc CDN runtime bên ngoài.
- Thu hẹp CSP sau khi loại bỏ các request font/avatar không cần thiết từ CDN.
...
- Sửa lỗi CSP chặn Google Fonts và DiceBear khiến console phát sinh hàng loạt lỗi fetch khi tải dashboard và avatar.
...
- Cài sạch npm ci đã loại thêm các cảnh báo deprecated cũ của ExcelJS mà không làm thay đổi API ứng dụng.
...
- ExcelJS dùng các nhánh archiver, fast-csv và unzipper hiện hành đã kiểm thử tương thích để loại bỏ nhóm glob/inflight/lodash.isequal/fstream cũ.
...
- Xác nhận round-trip XLSX/CSV của các dependency Excel đã thay thế, bảo đảm đọc/ghi workbook vẫn hoạt động.
...
- Các luồng nhập và xem Excel được chặn tệp rỗng, sai phần mở rộng hoặc vượt giới hạn kích thước trước khi parser chạy.
- Luồng nhập kết quả MassHunter vẫn nhận workbook lớn hơn với giới hạn 50 MB và giữ xử lý trong Web Worker khi trình duyệt hỗ trợ.

#### ✨ Tính Năng Mới

- Bổ sung helper bảo mật dùng chung cho kiểm tra kích thước/phần mở rộng và cấu hình SheetJS tối thiểu cho dữ liệu Excel không tin cậy.
- Áp dụng cấu hình đọc an toàn cho nhập danh mục chất chuẩn, nhật ký sử dụng, mô tả mẫu, chỉ tiêu gốc và kết quả MassHunter.

#### ⚡ Cải Tiến & Tối Ưu

- Excel Viewer giới hạn workbook 50 MB, vẫn giữ công thức/style cần cho chế độ chỉ xem nhưng tắt VBA, file phụ, HTML và dependency metadata không cần thiết.
- Luồng import chỉ tiêu gốc chuyển sang async/await và luôn dọn trạng thái loading, kể cả khi đọc tệp hoặc parse workbook thất bại.
- Bổ sung test hồi quy cho giới hạn tệp, phần mở rộng và các tùy chọn parser được phép dùng.

#### 🐛 Sửa Lỗi Hệ Thống

- Ngăn các luồng Excel phụ bỏ qua giới hạn 10 MB đã áp dụng cho nhập danh mục chuẩn.
- Ngăn tệp quá lớn hoặc workbook rỗng đi vào SheetJS/ExcelJS, giảm nguy cơ treo giao diện và giảm bề mặt xử lý dữ liệu thừa.
- Giữ lại giới hạn 50 MB riêng cho workbook MassHunter để không làm hỏng các tệp có chromatogram artifact.
...
- Lưới lịch tháng được thu gọn chiều cao nhưng tăng cỡ chữ tên, ngày, giờ và trạng thái để quan sát nhanh hơn trên cùng một màn hình.
- Thống kê lịch trực bổ sung so sánh từng người với mức bình quân, giúp nhận biết ngay ai đang trực nhiều hơn, ít hơn hoặc tương đối cân bằng.

#### ✨ Tính Năng Mới

- Màn hình Thống kê có ba phạm vi: kỳ đang chọn, cả năm và toàn bộ dữ liệu lịch trực đã lưu.
- Mỗi nhân sự có chỉ số phần trăm lệch so với bình quân và số lượt chênh lệch tuyệt đối trong cùng phạm vi thống kê.
- Nhân sự đang hoạt động chưa từng được xếp ca vẫn xuất hiện trong thống kê với 0 lượt để phản ánh đầy đủ mức phân bổ.

#### ⚡ Cải Tiến & Tối Ưu

- Các ô lịch tháng giảm khoảng trống và tăng độ dễ đọc của tên nhân sự, giờ trực và nhãn trạng thái trên Dashboard và màn hình Lịch trực.
- Thông tin mã nhân viên được diễn đạt rõ: có mã sẽ hiển thị theo dạng Mã NV, chưa có mã sẽ ghi Chưa khai báo mã nhân viên thay cho nhãn mơ hồ Chưa có mã.
- Bình quân lượt trực được tính trên toàn bộ nhân sự đang hoạt động trong thống kê, kể cả người có 0 lượt, để chỉ số cân bằng không bị lệch.
