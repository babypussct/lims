...
import { getAuth, signInWithEmailAndPassword, signInWithPopup, signInWithRedirect, linkWithPopup, linkWithRedirect, linkWithCredential, getRedirectResult, signOut, onAuthStateChanged, GoogleAuthProvider, unlink, updatePassword, sendPasswordResetEmail, setPersistence, browserSessionPersistence, browserLocalPersistence, EmailAuthProvider, reauthenticateWithCredential } from 'firebase/auth';
import { doc, setDoc, serverTimestamp, onSnapshot, updateDoc, arrayRemove, collection, query, limit, getDocs, writeBatch } from 'firebase/firestore';
...
export class AuthService {
    ...
    constructor() {
        ...
        this.currentUserUid = computed(() => this.currentUser()?.uid ?? null);
        this.isAuthReady = signal(false);
        /** true trong khi đang xử lý token trả về từ Google redirect — dùng để ẩn màn hình Login */
        this.isProcessingRedirect = signal(false);
        /** Lỗi Google redirect gần nhất, hiển thị lại trên màn hình đăng nhập. */
        this.googleRedirectError = signal(null);
        /** Email của tài khoản local cần xác thực để hoàn tất liên kết Google. */
        this.pendingGoogleLinkEmail = signal(null);
        this.pendingGoogleCredential = null;
        this.authProviderIds = signal([]);
        this.hasGoogleProvider = computed(() => this.authProviderIds().includes('google.com'));
        this.hasPasswordProvider = computed(() => this.authProviderIds().includes('password'));
        this.requiresCurrentPassword = computed(() => {
         ...
         */
        this.needsPasswordSetup = computed(() => {
            const profile = this.currentUser();
            if (!profile || !this.hasGoogleProvider())
                return false;
        ...
        this.passwordSetupRequested = signal(false);
        /**
         ...
         */
        this.isSettingPassword = signal(false);
        this.isPasswordSetupOpen = computed(() => this.passwordSetupRequested() || (this.needsPasswordSetup() && !this.isSettingPassword()));
        /** Quản lý trạng thái mở Modal Quên mật khẩu toàn cục */
        this.forgotPasswordRequested = signal(false);
        ...
        const persistenceReady = setPersistence(this.auth, rememberSession ? browserLocalPersistence : browserSessionPersistence).catch((err) => {
    ...
    processGoogleRedirectResult(hasPendingGoogleRedirect, hasPendingGoogleLink) {
        void getRedirectResult(this.auth)
            ...
            if (hasPendingGoogleRedirect && this.capturePendingGoogleCredential(error)) {
                this.googleRedirectError.set(`Email ${this.pendingGoogleLinkEmail()} đã có tài khoản mật khẩu. Nhập mật khẩu hiện tại để liên kết Google.`);
            }
            else if (hasPendingGoogleLink) {
    ...
    updatePersistence(rememberSession) {
     ...
     */
    async setLocalPassword(password, currentPassword) {
        ...
        if (requireCurrentPassword) {
            ...
            const currentCredential = EmailAuthProvider.credential(this.normalizeAuthEmail(firebaseUser.email), currentPassword);
        ...
        try {
            if (isUpdatingPassword) {
                ...
                const credential = EmailAuthProvider.credential(this.normalizeAuthEmail(firebaseUser.email), password);
    ...
    async unlinkProvider(providerId) {
        ...
        this.updateAuthProviderState(firebaseUser);
        if (!this.authProviderIds().includes(providerId))
            throw new Error('Phương thức đăng nhập này chưa được liên kết.');
        if (!this.canUnlinkProvider(providerId))
            throw new Error('Không thể hủy liên kết phương thức cuối cùng. Vui lòng thiết lập phương thức khác trước.');
     ...
     */
    async linkPendingGoogleAccount(password) {
    ...
    validateNewPassword(password, requireStrongLength = true) {
        const minLength = requireStrongLength ? 8 : 1;
        if (!password || password.length < minLength) {
            throw this.createAuthError('auth/weak-password', requireStrongLength ? 'Mật khẩu phải có ít nhất 8 ký tự.' : 'Vui lòng nhập mật khẩu.');
        }
    }
    ...
    updateAuthProviderState(firebaseUser) {
        this.authProviderIds.set((firebaseUser.providerData || [])
            .map(provider => provider.providerId)
            .filter((providerId) => !!providerId));
    }
    /** Chỉ lưu trạng thái onboarding; Firebase Auth vẫn là nguồn xác thực thật. */
    async markLocalPasswordConfigured(firebaseUser) {
        const profileRef = doc(this.fb.db, `artifacts/${this.fb.APP_ID}/users/${firebaseUser.uid}`);
        await setDoc(profileRef, {
    ...
    async recordPasswordChangedAt(firebaseUser) {
        const profileRef = doc(this.fb.db, `artifacts/${this.fb.APP_ID}/users/${firebaseUser.uid}`);
    ...
    capturePendingGoogleCredential(error) {
        const credential = (error?.credential ||
            GoogleAuthProvider.credentialFromError?.(error) || null);
    ...
    async validateCompletedGoogleLink(firebaseUser) {
        ...
        if (!googleEmail || googleEmail !== accountEmail) {
            ...
            throw this.createAuthError('auth/google-email-mismatch', 'Tài khoản Google phải dùng đúng email đang là ID đăng nhập LIMS.');
        }
        this.updateAuthProviderState(firebaseUser);
        await this.markLocalPasswordConfigured(firebaseUser);
    }
    googleLinkErrorMessage(error) {
        const code = typeof error === 'string' ? error : error?.code;
        switch (code) {
            ...
            try {
                if (snap.exists()) {
                    ...
                    if ((data.email || '').toLowerCase() === 'oneloveonepeopleforever@gmail.com' && data.role !== 'manager') {
                        ...
                        }).catch((e) => console.error("Could not auto-promote Super Admin in Firestore", e));
                    }
    ...
    async approveAuthSession(sessionId, _email, _encryptedPass, _deviceName) {
        console.warn('[Auth] approveAuthSession is deprecated. Use /api/qr/approve API endpoint.');
    }
    async verifyPassword(email, pass) {
    ...
    syncRolesConfig() {
        ...
        if (this.rolesInitPromise)
            return this.rolesInitPromise;
        ...
        const startPromise = (async () => {
            ...
            const unsubscribe = onSnapshot(rolesRef, (snap) => {
                const config = {};
                snap.forEach((doc) => {
                    const data = doc.data();
        ...
        const trackedPromise = startPromise.finally(() => {
            if (this.rolesInitPromise !== trackedPromise)
                return;
            this.rolesInitPromise = null;
    ...
    async initializeDefaultRolesIfNeeded() {
        try {
            const rolesRef = collection(this.fb.db, `artifacts/${this.fb.APP_ID}/roles_config`);
    ...
    canApprove() { return this.hasPermission(PERMISSIONS.SOP_APPROVE); }
    canApproveStandards() { return this.hasPermission(PERMISSIONS.STANDARD_APPROVE); }
    canViewStandardLogs() { return this.hasPermission(PERMISSIONS.STANDARD_LOG_VIEW); }
    canDeleteStandardLogs() {
    ...
    canEditInventory() { return this.hasPermission(PERMISSIONS.INVENTORY_EDIT); }
    canViewInventory() { return this.hasPermission(PERMISSIONS.INVENTORY_VIEW); }
    canEditSop() { return this.hasPermission(PERMISSIONS.SOP_EDIT); }
    canViewSop() { return this.hasPermission(PERMISSIONS.SOP_VIEW); }
    canEditRecipes() { return this.hasPermission(PERMISSIONS.RECIPE_EDIT); }
    canViewRecipes() { return this.hasPermission(PERMISSIONS.RECIPE_VIEW); }
    canEditStandards() { return this.hasPermission(PERMISSIONS.STANDARD_EDIT); }
    canAssignStandards() { return this.hasPermission(PERMISSIONS.STANDARD_EDIT) || this.hasPermission(PERMISSIONS.STANDARD_APPROVE); }
    canViewStandards() { return this.hasPermission(PERMISSIONS.STANDARD_VIEW); }
    canViewReports() { return this.hasPermission(PERMISSIONS.REPORT_VIEW); }
    canManageSystem() { return this.hasPermission(PERMISSIONS.USER_MANAGE); }
    /** Chạy Smart Batch hoặc Trạm pha chế (thao tác tiêu hao kho thực tế) */
    canRunBatch() { return this.hasPermission(PERMISSIONS.BATCH_RUN); }
