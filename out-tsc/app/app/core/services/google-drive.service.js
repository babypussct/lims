...
export class GoogleDriveService {
    ...
    setCachedFolder(folderId, items) {
    ...
    constructor() {
        ...
        this.isAuthenticated = signal(false); // true once an access token is obtained
        // Thêm bộ nhớ đệm (caching) cho thư mục
        this.folderCache = new Map();
        this.CACHE_TTL = 3 * 60 * 1000; // Cache trong 3 phút (3 * 60 * 1000 ms)
     ...
     */
    waitForGoogleGlobal(resolve, reject) {
        // Immediate check
        ...
        const interval = setInterval(() => {
            ...
            if (attempts >= maxAttempts) {
                clearInterval(interval);
                reject(new Error('Google Identity Services tải xong nhưng không khởi tạo được (timeout 30s).\n' +
        ...
        return new Promise((resolve, reject) => {
            ...
            this.currentErrorCallback = (error) => {
                ...
                if (error?.type === 'popup_closed') {
                    ...
                    reject(new Error('Không thể mở popup đăng nhập Google.\n' +
     ...
     */
    authenticateSync(onSuccess, onError, allowRedirect = true) {
    ...
    requestNativePopup(onSuccess, onError, allowRedirect) {
        const timeout = setTimeout(() => {
            cleanup();
     ...
     */
    async uploadToDrive(file, fileName, token) {
        const config = environment.googleDrive;
        const metadata = {
            name: fileName,
        ...
        const doUpload = async (authToken) => {
            ...
            if (!res.ok) {
                ...
                if (res.status === 403) {
                    throw new Error(`Bạn không có quyền tải tệp lên thư mục Google Drive.\n` +
     ...
     */
    async setPublicPermission(fileId, token) {
        try {
     ...
     */
    async downloadPublicFile(fileId, signal) {
        const config = environment.googleDrive;
        if (!config?.apiKey) {
     ...
     */
    async exportPublicFile(fileId, mimeType, signal) {
        const config = environment.googleDrive;
        if (!config?.apiKey) {
            throw new Error('Chưa cấu hình Google Drive API Key trong environment.');
        }
     ...
     */
    static generateFileName(standardName, lotNumber, originalFileName) {
     ...
     */
    async getFolderContents(folderId, signal) {
        const config = environment.googleDrive;
        if (!config?.apiKey) {
            throw new Error('Chưa cấu hình Google Drive API Key trong environment.');
        }
    ...
    static { this.ɵfac = function GoogleDriveService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || GoogleDriveService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: GoogleDriveService, factory: GoogleDriveService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(GoogleDriveService, [{
        type: Injectable,
