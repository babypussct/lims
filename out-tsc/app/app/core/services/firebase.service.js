...
import { collection, getDocs, query, limit, doc, writeBatch, deleteDoc, setDoc, initializeFirestore, persistentLocalCache, persistentMultipleTabManager, updateDoc, getCountFromServer, where, Timestamp, serverTimestamp, clearIndexedDbPersistence, terminate } from 'firebase/firestore';
...
export class FirebaseService {
    constructor() {
        ...
        this.APP_ID_KEY = 'lims_app_id';
        // --- User Management (New) ---
        this.usersCacheTtlMs = 5 * 60 * 1000;
    ...
    checkSystemHealth() {
        ...
        const checks$ = collections.map(colName => {
            ...
            return from(getDocs(query(colRef, limit(1)))).pipe(map(() => ({
                collection: colName,
                path: path,
                status: 'Online',
    ...
    async updateUserPermissions(uid, role, permissions, roleId, customPermissions) {
    ...
    async fetchOldData(collectionName, daysOld) {
    ...
    async deleteDocsInBatch(collectionName, docIds) {
    ...
    async restoreArchivedData(collectionName, items) {
    ...
    async importData(jsonData) {
        if (!jsonData.sops || !jsonData.inventory)
    ...
    async updateMetadata(moduleKey, event) {
    ...
    buildMetadataUpdate(moduleKey, event) {
        ...
        if (event?.action)
            syncEvent['action'] = event.action;
        if (event?.message)
            syncEvent['message'] = event.message;
        if (event?.targetId)
            syncEvent['targetId'] = event.targetId;
        return {
            [moduleKey]: version,
    ...
    static { this.ɵfac = function FirebaseService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || FirebaseService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: FirebaseService, factory: FirebaseService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(FirebaseService, [{
        type: Injectable,
