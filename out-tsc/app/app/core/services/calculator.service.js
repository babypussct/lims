...
const ChemHelper = {
    dilute: (cStock, cTarget, vTarget) => {
        if (cStock === 0)
            return 0;
        return (cTarget * vTarget) / cStock;
    },
    molarMass: (molarity, mw, volMl) => {
...
export class CalculatorService {
     ...
     */
    calculateSopNeeds(sop, inputValues, safetyMargin = 0, inventoryMap = {}, recipeMap = {}, safetyConfig) {
        ...
        allFormulas.forEach(formula => {
            ...
            if (matches) {
                matches.forEach(match => {
                    if (isNaN(parseFloat(match)) && !knownGlobals.includes(match) && typeof Math[match] !== 'function' && !(match in ChemHelper)) {
        ...
        if (sop.variables) {
            ...
            if (resolvedKeys.size < varEntries.length) {
                varEntries.forEach(([key]) => { if (!resolvedKeys.has(key))
                    ctx[key] = 0; });
            }
        }
        ...
        return sop.consumables.map(item => {
            ...
            if (isComposite || isSharedRecipe) {
                // Parent recipes/composites don't have their own margin, it's applied to ingredients
                appliedMargin = 0;
            }
            ...
            const breakdown = rawIngredients.map(ing => {
                ...
                const baseAmountPerBatch = baseQty * ing.amount;
                const amountPerBatch = baseAmountPerBatch * (1 + ingMargin / 100);
                if (ingStockItem) {
    ...
    calculateCapacity(sop, mode = 'marginal', inventoryMap = {}, recipeMap = {}) {
        const capacityInputs = {};
        sop.inputs.forEach(inp => { capacityInputs[inp.var] = inp.default; });
        if (mode === 'marginal') {
            sop.inputs.forEach(inp => {
                const v = inp.var.toLowerCase();
                // Chỉ ghi đè các biến đại diện cho SỐ LƯỢNG (number/count), bỏ qua các biến thể tích/khối lượng (vol, weight, mass...)
                if (v === 'num_samples' || v === 'n_samples' || v === 'samples' || (v.includes('num') && v.includes('sample'))) {
                    capacityInputs[inp.var] = 1;
                }
                else if (v === 'num_qc' || v === 'n_qc' || v === 'qc' || (v.includes('num') && v.includes('qc'))) {
                    capacityInputs[inp.var] = 0;
                }
            });
        }
        ...
        aggregatedNeeds.forEach((totalNeed, id) => {
            ...
            if (!stockItem) {
                ...
                if (maxBatches > 0) {
                    maxBatches = 0;
                    limitingFactor = id + ' (Không có trong kho)';
                }
            }
            else {
    ...
    static { this.ɵfac = function CalculatorService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || CalculatorService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: CalculatorService, factory: CalculatorService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(CalculatorService, [{
        type: Injectable,
