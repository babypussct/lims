...
export class PubchemService {
    async fetchGHS(query) {
        ...
        try {
            ...
            const hazardStatements = new Set();
            const precautionaryStatements = new Set();
            const extractGhsRecursive = (obj) => {
                if (typeof obj === 'string') {
                    const matchIcon = obj.match(/GHS0[1-9]/g);
                    if (matchIcon)
                        matchIcon.forEach(m => pictograms.add(m));
                    const matchH = obj.match(/^H[0-9]{3}.+?(?=\[|$)/g);
                    if (matchH)
                        matchH.forEach(m => hazardStatements.add(m.trim().replace(/\s*\([^)]*\)\s*:/, ':')));
                    const matchP = obj.match(/^P[0-9]{3}.+?(?=\[|$)/g);
                    if (matchP)
                        matchP.forEach(m => precautionaryStatements.add(m.trim().replace(/\s*\([^)]*\)\s*:/, ':')));
                }
     ...
     */
    async getChemicalInfo(identifier) {
        if (!identifier)
            return null;
        // Clean identifier
        ...
        try {
            ...
            const propertyRes = await fetch(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encodedIdentifier}/property/Title,IUPACName/JSON`);
            const propertyData = propertyRes.ok ? await propertyRes.json() : null;
            const properties = propertyData?.PropertyTable?.Properties?.[0] ?? {};
            const synonymRes = await fetch(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encodedIdentifier}/synonyms/JSON`);
            const synonymData = synonymRes.ok ? await synonymRes.json() : null;
            const rawSynonyms = synonymData?.InformationList?.Information?.[0]?.Synonym ?? [];
            ...
            if (!commercialName)
                return null;
            const synonyms = normalizeChemicalNames([properties.IUPACName || '', ...rawSynonyms], [commercialName, sanitizedIdentifier]).slice(0, 50);
    ...
    static { this.ɵfac = function PubchemService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || PubchemService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: PubchemService, factory: PubchemService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(PubchemService, [{
        type: Injectable,
        args: [{ providedIn: 'root' }]
    }], null, null); })();
//# sourceMappingURL=pubchem.service.js.map