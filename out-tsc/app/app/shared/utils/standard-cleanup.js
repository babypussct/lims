...
export function assessCasNumber(value) {
    const raw = value?.trim() ?? '';
    if (!raw)
        return { quality: 'missing', normalizedCas: null, reason: 'Chưa có số CAS.' };
 ...
 */
export function suggestCasCorrection(value) {
...
export function normalizeProductNameTypography(value) {
    return value
        .trim()
        .replace(/\s+/gu, ' ')
...
export function formatStandardProductName(value) {
...
export function detectStandardForm(name) {
...
export function assessCleanupGroup(standards) {
    ...
    const reasons = [];
    if (names.size > 1)
        reasons.push('Nhóm CAS đang có nhiều tên sản phẩm khác nhau.');
    if (units.size > 1)
        reasons.push('Các hồ sơ trong nhóm sử dụng đơn vị khác nhau.');
    if (forms.length > 1)
        reasons.push('Nhóm chứa nhiều dạng chất chuẩn khác nhau.');
    if (forms.includes('mixture'))
        reasons.push('Có hỗn hợp hoặc chuẩn đa thành phần.');
    if (forms.includes('solution'))
        reasons.push('Có dung dịch hoặc thông tin nồng độ/dung môi cần được giữ nguyên.');
    if (forms.includes('isotope'))
        reasons.push('Có chất đánh dấu đồng vị; kiểu chữ và chỉ số mang ý nghĩa khoa học.');
    if (forms.includes('salt_or_hydrate'))
        reasons.push('Có dạng muối hoặc hydrat cần được giữ nguyên.');
    ...
    const level = hasStructuralConflict
        ? 'high'
        : (names.size > 1 || hasComplexForm ? 'medium' : 'low');
    if (reasons.length === 0)
        reasons.push('Các hồ sơ có cùng tên và cùng dạng sản phẩm.');
    return {
        level,
        reasons,
        forms,
...
//# sourceMappingURL=standard-cleanup.js.map