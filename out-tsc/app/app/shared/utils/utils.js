...
export function getStandardizedAmount(amount, fromUnit, toUnit) {
    if (!fromUnit || !toUnit)
        return amount;
    if (fromUnit.toLowerCase() === toUnit.toLowerCase())
        return amount;
    const findKey = (u) => Object.keys(UNIT_DATA).find(k => k.toLowerCase() === u.toLowerCase().trim());
...
export function parseQuantityInput(inputStr, baseUnit) {
    if (!inputStr)
        return null;
...
export function formatSmartUnit(amount, unit) {
    if (amount === 0)
        return `0 <span class="text-[10px] text-slate-400">${unit}</span>`;
    if (unit === 'ml') {
        if (amount >= 1000)
            return `${formatNum(amount / 1000)} <span class="text-[10px] text-slate-400">L</span>`;
        if (amount < 1 && amount > 0)
            return `${formatNum(amount * 1000)} <span class="text-[10px] text-slate-400">µl</span>`;
    }
    if (unit === 'g') {
        if (amount >= 1000)
            return `${formatNum(amount / 1000)} <span class="text-[10px] text-slate-400">kg</span>`;
        if (amount < 1 && amount > 0)
            return `${formatNum(amount * 1000)} <span class="text-[10px] text-slate-400">mg</span>`;
    }
 ...
 */
export function getAvatarUrl(name, style = 'bottts-neutral', photoUrl) {
 ...
 */
export function formatSampleList(samplesInput, options = {}) {
    if (!samplesInput)
        return '';
    let samples = [];
    ...
    for (let i = 1; i < parsed.length; i++) {
        ...
        if (isSequential(prev, curr)) {
            ...
            if (count === 1) {
                ranges.push(start.orig);
            }
            else if (count === 2) {
                ranges.push(`${start.orig}; ${prev.orig}`);
            }
            else {
                ranges.push(`${start.orig} -> ${prev.orig}`);
            }
 ...
 */
export function calculateSimilarityScore(query, std) {
    if (!query || !std)
        return 0;
    let score = 0;
    ...
    if (lotClean && lotClean.length > 2 && lotClean === queryClean)
        score += 200;
    // Query contains the standard's lot (Useful when parsing Filename)
    else if (lotClean && lotClean !== 'na' && lotClean.length > 2 && queryClean.includes(lotClean))
        score += 100;
    if (codeClean && codeClean.length > 2 && codeClean === queryClean)
        score += 150;
    else if (codeClean && codeClean !== 'na' && codeClean.length > 2 && queryClean.includes(codeClean))
        score += 80;
    if (casClean && casClean.length > 4 && casClean === queryClean)
        score += 100;
    else if (casClean && casClean.length > 4 && queryClean.includes(casClean))
        score += 40;
    // Medium Priority: Name matching
    if (nameClean && nameClean === queryClean)
        score += 120;
    else if (nameClean && nameClean.length > 5 && queryClean.includes(nameClean.substring(0, 10)))
        score += 50;
    else if (nameClean && nameClean.length > 3 && queryClean.includes(nameClean.substring(0, 5)))
        score += 20;
    // Medium/Low Priority: Chemical Name & Manufacturer
    if (chemicalClean && chemicalClean === queryClean)
        score += 60;
    else if (chemicalClean && chemicalClean.length > 5 && queryClean.includes(chemicalClean.substring(0, 10)))
        score += 30;
    if (manufacturerClean && manufacturerClean === queryClean)
        score += 30;
    // Reverse inclusion (Standard's property contains the query) - Used for global SEARCH typed by user
    if (queryClean.length >= 2) {
        if (lotClean.includes(queryClean))
            score += 60;
        if (codeClean.includes(queryClean))
            score += 50;
        if (nameClean.includes(queryClean))
            score += 40;
        if (chemicalClean.includes(queryClean))
            score += 20;
    }
    return score;
}
// --- STANDARD UI HELPERS ---
export function getExpiryBarClass(dateStr) {
    if (!dateStr)
        return 'bg-slate-300';
...
export function getExpiryStatusClass(dateStr) {
    if (!dateStr)
...
export function getStorageInfo(condition) {
    if (!condition)
        return [];
    const items = [];
...
export function getExpiryTimeClass(dateStr) {
    if (!dateStr)
...
export function getExpiryTimeLeft(dateStr) {
    if (!dateStr)
        return '';
    const exp = parseStandardDate(dateStr);
    if (exp === null)
...
export function getStandardStatus(std) {
    if (std.status === 'IN_USE')
        return { label: 'Đang dùng', class: 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/50' };
    if (std.status === 'DEPLETED' || std.current_amount <= 0)
        return { label: 'Sử dụng hết', class: 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700' };
    ...
    if (std.has_pending_request && !std.current_holder && !std.current_request_id)
        return { label: 'Chờ duyệt', class: 'bg-orange-50 text-orange-600 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800/50' };
    if (!std.expiry_date)
        return { label: 'Chưa rõ hạn', class: 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50' };
    const exp = parseStandardDate(std.expiry_date);
    if (exp === null)
        return { label: 'Hạn không hợp lệ', class: 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800/50' };
    const today = startOfLocalDay();
    if (isStandardExpired(std.expiry_date))
        return { label: 'Hết hạn SD', class: 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800/50' };
    const diffDays = (exp - today) / (1000 * 3600 * 24);
    if (diffDays < 180)
        return { label: 'Sắp hết hạn', class: 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50' };
    if ((std.current_amount / (std.initial_amount || 1)) <= 0.2)
        return { label: 'Sắp hết hàng', class: 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50' };
...
export function getSafeGoogleUrl(url, type) {
    if (!url)
        return '';
