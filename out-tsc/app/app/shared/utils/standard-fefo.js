 ...
 */
export function parseStandardDate(value) {
...
export function isStandardExpired(expiryDate, now = new Date()) {
 ...
 */
export function canAssign(std, now = new Date()) {
    if (!std || std._isDeleted)
        return false;
    if (std.status === 'IN_USE' || std.status === 'DEPLETED' || std.status === 'DELETED')
        return false;
    if (std.current_holder || std.current_request_id)
        return false;
    ...
    if (std.expiry_date) {
        ...
        if (expiry === null || expiry < startOfLocalDay(now))
            return false;
    }
    return true;
}
/** A FEFO candidate must also be unreserved by another pending request. */
export function isFefoCandidate(std, now = new Date()) {
    return canAssign(std, now) && !std.has_pending_request;
}
export function getFefoUnavailableReason(std, now = new Date()) {
    if (std._isDeleted)
        return 'Đã xóa';
    if (std.status === 'IN_USE' || std.current_holder || std.current_request_id)
        return 'Đang sử dụng';
    ...
    if (std.has_pending_request)
        return 'Chờ duyệt';
    if (std.expiry_date && parseStandardDate(std.expiry_date) === null)
        return 'Hạn dùng không hợp lệ';
    if (isStandardExpired(std.expiry_date, now))
        return 'Hết hạn';
    return null;
}
function amountForSort(std) {
    const amount = Number(std.current_amount);
    return Number.isFinite(amount) ? amount : Number.POSITIVE_INFINITY;
}
function identityForSort(std) {
 ...
 */
export function compareStandardsByFefo(a, b, now = new Date()) {
...
export function sortStandardsByFefo(standards, now = new Date()) {
    return [...standards].sort((a, b) => compareStandardsByFefo(a, b, now));
}
export function getSameStandardLots(current, standards, includeCurrent = true) {
    const groupName = normalizeStandardName(current.name);
    if (!groupName)
        return [];
    return standards.filter(std => !std._isDeleted &&
        (includeCurrent || std.id !== current.id) &&
        normalizeStandardName(std.name) === groupName);
}
export function getFefoPriorityStandard(current, standards, now = new Date()) {
...
export function getFefoPredecessor(current, standards, now = new Date()) {
    if (!isFefoCandidate(current, now))
        return null;
    const priority = getFefoPriorityStandard(current, standards, now);
    return priority && priority.id !== current.id ? priority : null;
}
export function isFefoPriorityStandard(current, standards, now = new Date()) {
    const priority = getFefoPriorityStandard(current, standards, now);
    return !!priority && priority.id === current.id;
}
//# sourceMappingURL=standard-fefo.js.map