...
export function normalizeStandardUnit(unit) {
    const cleaned = (unit || '')
        .trim()
        .toLowerCase()
        .replace(/μ/g, 'µ')
...
export function parseStandardQuantity(value, normalizedUnit) {
    const targetUnit = normalizeStandardUnit(normalizedUnit);
    if (!targetUnit || !UNIT_DATA[targetUnit])
        return null;
    ...
    if (!Number.isFinite(amount) || !unit || !UNIT_DATA[unit])
        return null;
...
export function normalizePositiveStandardAmount(amount, fromUnit, toUnit, fieldLabel = 'Số lượng') {
...
export function normalizeNonNegativeStandardAmount(amount, fromUnit, toUnit, fieldLabel = 'Số lượng') {
    if (!Number.isFinite(amount) || amount < 0) {
...
export function reconcileStandardReturn(currentStock, normalizedLogAmounts, confirmedUsed, isDepleted) {
    if (!Number.isFinite(currentStock) || currentStock < 0)
        throw new Error('Tồn kho hiện tại không hợp lệ.');
    if (!Number.isFinite(confirmedUsed) || confirmedUsed < 0)
        throw new Error('Tổng lượng xác nhận không hợp lệ.');
    ...
    if (adjustmentAmount > currentStock + 1e-9)
        throw new Error('Không đủ lượng tồn kho để xác nhận trả chuẩn.');
...
//# sourceMappingURL=standard-amount.js.map