 ...
 */
export function formatChemicalName(name) {
    ...
    const uppercaseTokensToRestore = new Set((formatted.match(/[A-Z]+/gu) ?? []).filter(token => PROTECTED_UPPERCASE_IDENTIFIERS.has(token)));
    ...
    formatted = formatted.replace(/\p{L}[\p{L}\p{M}]*/gu, word => {
        ...
        if (currentIndex <= primaryWordIndex || word.length === 1)
            return word;
    ...
    formatted = formatted.replace(/\p{L}[\p{L}\p{M}]*/gu, word => {
        ...
        if (currentIndex !== primaryWordIndex)
            return word;
...
export function parseChemicalNames(value) {
    if (!value)
        return [];
    return value
        .split(/(?:\r?\n|;)+/u)
...
export function normalizeChemicalNames(names, excludedNames = []) {
    const excluded = new Set(Array.from(excludedNames, name => formatChemicalName(name).toLocaleLowerCase('en-US')).filter(Boolean));
    const seen = new Set();
    const result = [];
    for (const rawName of names) {
        const formatted = formatChemicalName(rawName);
        const key = formatted.toLocaleLowerCase('en-US');
        if (!formatted || excluded.has(key) || seen.has(key))
            continue;
        seen.add(key);
...
//# sourceMappingURL=chemical-name.js.map