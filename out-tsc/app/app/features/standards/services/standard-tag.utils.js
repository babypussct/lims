...
export function buildTagKey(source, sourceId) {
    return `${prefixForSource(source)}:${assertSafeTagId(sourceId, 'ID nhãn')}`;
}
export function parseTagKeyStrict(key) {
...
export function normalizeTagKeysStrict(keys, contextLabel = 'Nhãn') {
...
export function sanitizeLegacyTagKeys(keys) {
    if (!Array.isArray(keys))
        return [];
    const result = [];
    const seen = new Set();
...
export function assertTagLimit(tags, max, contextLabel) {
    if (tags.length > max) {
        throw new Error(`${contextLabel} tối đa ${max} nhãn (đã nhận ${tags.length}). Không tự động cắt nhãn.`);
    }
}
export function mergeUniqueTagKeys(existing, incoming) {
    return normalizeTagKeysStrict([...existing, ...incoming], 'Nhãn');
}
export function resolveReturnTagMerge(existingStandardTags, finalReturnTags) {
...
export function applyTagMode(currentTags, selectedTags, mode) {
...
export function summarizeStockByUnit(standards) {
...
export function formatStockSummary(summary) {
...
export function normalizeNafi6ChemicalMethodCode(value) {
...
export function compareChemicalMethodCodes(left, right) {
...
export function formatMethodOptionLabel(option) {
...
export function buildAccreditationMethodTagId(methodCode) {
...
export function deriveMethodSeries(methodCode) {
    const normalized = normalizeNafi6ChemicalMethodCode(methodCode);
    return normalized.slice('NAFI6/'.length).split('.')[0];
}
export function deriveDeviceCodesFromTagKeys(tagKeys, catalog) {
...
export const STANDARD_DEVICE_OPTIONS = [
    ['GC', 'GC', ['gc'], '#64748b'],
...
//# sourceMappingURL=standard-tag.utils.js.map