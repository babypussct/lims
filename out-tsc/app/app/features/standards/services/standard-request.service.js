...
import { doc, collection, getDoc, writeBatch, serverTimestamp, runTransaction, deleteField, query, where, limit, onSnapshot, getDocs } from 'firebase/firestore';
import { NotificationCenterService } from '../../../core/services/notification-center.service';
import { getStandardizedAmount } from '../../../shared/utils/utils';
import { canAssign } from '../../../shared/utils/standard-fefo';
import { normalizeNonNegativeStandardAmount, reconcileStandardReturn } from '../../../shared/utils/standard-amount';
import { assertPurchaseRequestTransition, assertStandardRequestTransition, canCompleteStandardReturn, normalizeLegacyStandardRequestStatus } from '../../../shared/utils/standard-workflow';
...
import { assertTagLimit, MAX_RETURN_TAGS, normalizeTagKeysStrict, resolveReturnTagMerge, sanitizeLegacyTagKeys, } from './standard-tag.utils';
...
export class StandardRequestService {
    ...
    _getCacheKey(roleKey) {
        return buildScopedDeltaKey(`lims_std_req_cache_${roleKey}_${this.fb.APP_ID}`, this.auth.getDeltaCacheScope());
    }
    _getCursorKey(roleKey) {
        return buildScopedDeltaKey(`lims_std_req_cursor_${roleKey}_${this.fb.APP_ID}`, this.auth.getDeltaCacheScope());
    }
    _getRoleKey() {
    ...
    startRequestsListener(callback) {
        ...
        return this.deltaSync.startSingletonListener({
    ...
    getRequestsFromCache() {
        ...
        return (this.deltaSync.getMemCache(cacheKey)
            ?? this.deltaSync.getCache(cacheKey))
            .filter(request => !request._isDeleted)
            .map(request => ({ ...request, status: normalizeLegacyStandardRequestStatus(request.status) }));
    }
    /**
     ...
     */
    async getRequestsFromListenerCache() {
        const cacheKey = this._getCacheKey(this._getRoleKey());
        if (!this.deltaSync.getSingletonStatus(cacheKey))
            return null;
        if (!await this.deltaSync.waitForSingletonReady(cacheKey))
            return null;
    ...
    listenToPendingPurchaseRequests(callback) {
        ...
        return onSnapshot(q, (snapshot) => {
            this.readMonitor.record('onSnapshot', `artifacts/${this.fb.APP_ID}/purchase_requests`, isFirstSnapshot
                ? snapshot.size
    ...
    async createRequest(request, isAssign = false) {
        ...
        if (request.expectedAmount !== undefined && (!Number.isFinite(request.expectedAmount) || request.expectedAmount < 0)) {
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            ...
            const standard = { ...stdDoc.data(), id: stdDoc.id };
            ...
            const { finalSopTags: _finalSopTags, tagMergeStatus: _tagMergeStatus, tagMergeWarning: _tagMergeWarning, ...requestWithoutAdminDecision } = request;
    ...
    async updateRequestStatus(requestId, status, updates = {}) {
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            ...
            const freshRequest = { ...reqDoc.data(), id: reqDoc.id };
            if (freshRequest._isDeleted)
                throw new Error('Yêu cầu đã bị xóa.');
            const isOwner = freshRequest.requestedBy === currentUser.uid;
            const isApprover = this.auth.canAssignStandards();
            if (!isOwner && !isApprover)
                throw new Error('Bạn không có quyền cập nhật yêu cầu này.');
            if (status === 'REJECTED' && !isApprover)
                throw new Error('Chỉ người duyệt mới có thể từ chối yêu cầu.');
            ...
            const stdDoc = await transaction.get(stdRef);
            const stockUnit = stdDoc.exists() ? (stdDoc.data().unit || 'mg') : 'mg';
            const safeUpdates = {};
            if (isApprover) {
                for (const key of ['purpose', 'expectedAmount', 'rejectionReason']) {
                    if (updates[key] !== undefined)
                        safeUpdates[key] = updates[key];
                }
            }
            if (updates.reportedDepleted !== undefined)
                safeUpdates['reportedDepleted'] = Boolean(updates.reportedDepleted);
            if (status === 'PENDING_RETURN') {
                const reported = normalizeNonNegativeStandardAmount(Number(updates.totalAmountUsed ?? freshRequest.totalAmountUsed ?? 0), stockUnit, stockUnit, 'Tổng lượng đã dùng');
            ...
            reqData = { ...freshRequest, status, ...safeUpdates };
        });
        if (reqData) {
    ...
    // ─── Dispense Standard ────────────────────────────────────────────────────────
    async dispenseStandard(requestId, standardId, approverId, approverName, isAssign = false) {
        const currentUser = this.auth.currentUser();
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            ...
            const standard = { ...stdData, id: stdDoc.id };
            if (!canAssign(standard)) {
        ...
        if (reqData) {
            if (!isAssign) {
                await this.crud.logGlobalActivity('APPROVE_STANDARD_REQUEST', `Duyệt cấp chuẩn: ${reqData.standardName}`, requestId);
                await this.notificationCenter.publish({
                    ...
                    message: `Yêu cầu mượn lô chuẩn ${reqData.standardName} đã được phê duyệt. Xin hãy bảo quản cẩn thận!`,
                ...
                if (currentUser.uid !== reqData.requestedBy) {
                    await this.notificationCenter.publish({
                        eventId: `standard-request:${requestId}:direct-assignment:recipient`,
                        recipientUid: reqData.requestedBy, senderUid: currentUser.uid,
                        senderName: currentUser.displayName || 'Quản trị viên',
                        type: 'REQUEST_APPROVED', title: 'Được cấp chuẩn',
                        message: `Quản trị viên đã trực tiếp cấp cho bạn lô chuẩn ${reqData.standardName}. Xin hãy bảo quản cẩn thận!`,
                        targetId: standardId, actionUrl: `/standards/${standardId}`,
                        channels: ['inbox', 'push']
                    });
                }
                ...
                await this.notificationCenter.publish({
                    ...
                    message: `Quản trị viên ${currentUser.displayName} vừa gán trực tiếp lô chuẩn ${reqData.standardName} cho ${reqData.requestedByName}.`,
                    targetId: standardId, actionUrl: `/standards/${standardId}`,
                    channels: ['inbox', 'push']
                });
            }
        ...
        try {
            const staleQuery = query(collection(this.fb.db, `artifacts/${this.fb.APP_ID}/standard_requests`), where('standardId', '==', standardId), where('status', '==', 'PENDING_APPROVAL'));
            ...
            console.warn('[StandardRequestService] cleanup stale requests failed:', e);
        }
        ...
        try {
            ...
            if (freshSnap.exists()) {
                const freshStd = { id: freshSnap.id, ...freshSnap.data() };
                this.cache._mergeAndSave([freshStd], []);
            }
        }
        catch (e) {
            console.warn('[StandardRequestService] post-dispense cache merge failed:', e);
        }
    }
    // ─── Return Standard ──────────────────────────────────────────────────────────
    async returnStandard(requestId, standardId, receiverId, receiverName, isDepleted = false, amountUsed, unit, disposalReason, finalSopTags) {
        const currentUser = this.auth.currentUser();
        if (!currentUser || !this.auth.canAssignStandards()) {
        ...
        const reqRef = doc(this.fb.db, `artifacts/${this.fb.APP_ID}/standard_requests/${requestId}`);
        let reqData = null;
        let returnResult = { tagMergeStatus: 'NOT_REQUESTED' };
        await runTransaction(this.fb.db, async (transaction) => {
            const stdDoc = await transaction.get(stdRef);
            const reqDoc = await transaction.get(reqRef);
            if (!stdDoc.exists())
            ...
            if (finalSopTags !== undefined) {
                finalSopTags = this.tagCatalog.assertKnownOrExistingKeys(finalSopTags, reqData.sopTags || [], 'Nhãn Admin xác nhận');
            }
            let tagMerge = {
            ...
            const reconciliation = reconcileStandardReturn(stdData.current_amount || 0, normalizedLogAmounts, confirmedTotal, isDepleted);
            ...
            if (tagMerge.status === 'MERGED')
                standardUpdate['sop_tags'] = tagMerge.standardTags;
            ...
            if (finalSopTags !== undefined) {
                ...
                if (tagMerge.warning)
                    reqUpdateData['tagMergeWarning'] = tagMerge.warning;
                else
                    reqUpdateData['tagMergeWarning'] = deleteField();
            }
            if (disposalReason)
                reqUpdateData['disposalReason'] = disposalReason;
            const appendedLogs = [];
            const appendLog = (normalizedAmount, purpose) => {
                if (normalizedAmount <= 1e-9)
                    return;
                ...
                transaction.set(doc(this.fb.db, `artifacts/${this.fb.APP_ID}/standard_usages/${log.id}`), { ...log, lastUpdated: serverTimestamp() });
            };
            ...
            if (appendedLogs.length)
                reqUpdateData['usageLogs'] = [...currentLogs, ...appendedLogs];
        ...
        if (reqData) {
            await this.crud.logGlobalActivity('RETURN_STANDARD', `Nhận lại chuẩn: ${reqData.standardName}`, requestId);
        }
        // Merge ngay document mới vào cache KHÔNG xóa trước — để giữ nguyên toàn bộ danh sách chuẩn
        try {
            const freshSnap = await getDoc(stdRef);
            ...
            console.warn('[StandardRequestService] post-return cache merge failed:', e);
        }
        return returnResult;
    }
    // ─── Hard Delete Request ──────────────────────────────────────────────────────
    async hardDeleteRequest(request) {
        ...
        if (!request.id)
            throw new Error('Yêu cầu không có mã định danh.');
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            ...
            if (!reqDoc.exists())
                return;
            freshRequest = { ...reqDoc.data(), id: reqDoc.id };
            if (freshRequest._isDeleted || freshRequest.rolledBackAt)
                return;
        ...
        this.deltaSync.mergeSingletonCache(this._getCacheKey(this._getRoleKey()), [], [request.id]);
        if (didRollback && freshRequest) {
            await this.crud.logGlobalActivity('HARD_DELETE_REQUEST', `Rollback lịch sử yêu cầu: ${freshRequest.standardName} (Người yêu cầu: ${freshRequest.requestedByName})`, request.id);
        }
    }
    // ─── Purchase Requests ────────────────────────────────────────────────────────
    async createPurchaseRequest(req) {
        ...
        if (!req.standardId)
            throw new Error('Thiếu mã chuẩn cần mua.');
        if (!req.expectedAmount?.trim())
            throw new Error('Số lượng cần mua là bắt buộc.');
        if (req.priority && !['NORMAL', 'HIGH'].includes(req.priority))
            throw new Error('Mức ưu tiên không hợp lệ.');
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            ...
            if (!stdDoc.exists())
                throw new Error('Chuẩn không tồn tại.');
            const standard = { ...stdDoc.data(), id: stdDoc.id };
            if (standard.restock_requested)
                throw new Error('Chuẩn này đã có yêu cầu mua đang xử lý.');
            const newReq = {
                ...req,
                id,
                standardId: standard.id,
    ...
    async completePurchaseRequest(reqId, stdId, processedBy, processedByName) {
        ...
        await runTransaction(this.fb.db, async (transaction) => {
            const reqDoc = await transaction.get(reqRef);
            const stdDoc = await transaction.get(stdRef);
            if (!reqDoc.exists() || !stdDoc.exists())
                throw new Error('Yêu cầu mua hoặc chuẩn không tồn tại.');
            const freshRequest = reqDoc.data();
            if (freshRequest.standardId !== stdId)
                throw new Error('Yêu cầu mua không thuộc chuẩn đã chọn.');
    ...
    static { this.ɵfac = function StandardRequestService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || StandardRequestService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: StandardRequestService, factory: StandardRequestService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(StandardRequestService, [{
        type: Injectable,
        args: [{ providedIn: 'root' }]
    }], () => [], null); })();
//# sourceMappingURL=standard-request.service.js.map