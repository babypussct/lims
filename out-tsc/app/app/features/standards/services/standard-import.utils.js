...
export const STANDARD_IMPORT_ALLOWED_EXTENSIONS = ['xlsx', 'xlsm', 'csv'];
...
export function validateStandardImportFile(file) {
    const extension = file.name.split('.').pop()?.toLowerCase() || '';
    if (!STANDARD_IMPORT_ALLOWED_EXTENSIONS.includes(extension)) {
...
export function parseExcelDateDetailed(value) {
...
export function isActiveStandardIdentity(standard) {
    return standard._isDeleted !== true && standard.status !== 'DELETED';
}
function createStandardDocumentId(options, internalId, name, lot, rowNumber) {
...
export function buildSafeImportMetadata(parsed, presentFields) {
...
export function computeImportChanges(current, parsed, presentFields) {
...
export function countAtomicStandardImportWrites(items, existingIds) {
    return items.reduce((count, item) => count + 1 + (existingIds.has(item.parsed.id) ? 0 : (item.logs?.length || 0) * 2), 1);
}
function inferPackUnit(packText) {
    const lower = packText.toLowerCase();
    if (lower.includes('ml') || lower.includes('lít'))
        return 'ml';
    if (lower.includes('µg') || lower.includes('ug') || lower.includes('mcg'))
        return 'µg';
    if (lower.includes('kg'))
        return 'kg';
    if (lower.includes('g') && !lower.includes('mg') && !lower.includes('kg'))
        return 'g';
    if (lower.includes('µl') || lower.includes('ul'))
        return 'µl';
    return 'mg';
}
function parseInlineUsageLog(value, defaultDate, standardUnit) {
    const raw = String(value ?? '').trim();
    if (!raw)
        return {};
    ...
    if (dateResult.error)
        return { error: `Ngày trong nhật ký không hợp lệ: ${dateMatch?.[1]}.` };
    ...
    const labelledAmount = raw.match(/(?:lượng|kl|amount)(?:\s*(?:d[ùu]ng|c[âa]n|used))?\s*[:\s-]*([+-]?(?:\d+(?:[.,]\d*)?|[.,]\d+))\s*([a-zA-Zµμ]+(?:\s*\/\s*(?:chai|lọ|lo|vial|bottle|ampoule|ampule|ống|ong))?)?/iu);
    ...
    if (!amountToken)
        return { error: `Không đọc được lượng dùng từ "${raw}".` };
...
function buildHeaderMap(rawRows) {
    ...
    Object.keys(HEADER_ALIASES).forEach(field => {
        ...
        if (unique[0])
            mapping.set(field, unique[0]);
    });
    if (!mapping.has('name'))
        throw new Error('Thiếu cột bắt buộc "Tên chuẩn".');
    if (!mapping.has('initial_amount'))
        throw new Error('Thiếu cột bắt buộc "Khối lượng chai/Lượng ban đầu".');
    return mapping;
}
...
function existingCandidates(existingStandards, internalId, name, lot) {
    const activeStandards = existingStandards.filter(isActiveStandardIdentity);
    if (internalId) {
        const byInternalId = activeStandards.filter(standard => normalizeIdentity(standard.internal_id) === normalizeIdentity(internalId));
        if (byInternalId.length)
            return byInternalId;
    }
    if (name && lot) {
        return activeStandards.filter(standard => normalizeIdentity(standard.name) === normalizeIdentity(name) &&
            normalizeIdentity(standard.lot_number) === normalizeIdentity(lot));
    }
    return [];
}
export function parseStandardImportRows(rawRows, options) {
    if (!rawRows.length)
        throw new Error('Sheet không có dữ liệu.');
    const mapping = buildHeaderMap(rawRows);
    const presentFields = [...mapping.keys()];
    ...
    rawRows.forEach((rawRow, index) => {
        ...
        if (!internalId && !lot)
            errors.push('Cần có Số nhận diện hoặc Số lô để chống tạo trùng khi import lại.');
        else if (!internalId)
            warnings.push('Không có Số nhận diện; hệ thống dùng Tên + Số lô làm khóa đối chiếu.');
        ...
        if (receivedResult.error)
            errors.push(`Ngày nhận: ${receivedResult.error}`);
        if (expiryResult.error)
            errors.push(`Hạn sử dụng: ${expiryResult.error}`);
        const logs = [];
        ...
        if (hasExplicitCurrent && !currentParsed)
            errors.push('Lượng còn lại hoặc đơn vị không hợp lệ.');
        if (!Number.isFinite(current) || current < -1e-9)
            errors.push('Lượng còn lại không được âm.');
        ...
        if (candidates.length > 1)
            errors.push('Có nhiều chuẩn hiện hữu trùng khóa nhận diện; cần xử lý dữ liệu trùng trước khi import.');
...
//# sourceMappingURL=standard-import.utils.js.map