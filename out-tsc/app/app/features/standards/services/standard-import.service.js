...
import { doc, collection, getDocs, getDoc, writeBatch, serverTimestamp, runTransaction, query, where } from 'firebase/firestore';
...
import { STANDARD_IMPORT_MAX_ATOMIC_WRITES, buildSafeImportMetadata, computeImportChanges, countAtomicStandardImportWrites, isActiveStandardIdentity, parseExcelDateDetailed, parseStandardImportRows, validateStandardImportFile } from './standard-import.utils';
...
export class StandardImportService {
    ...
    async parseExcelWorkbook(file, sheetName) {
        ...
        if (!existingStandards.length)
            existingStandards = await this.cache.fetchAllAndCache();
    ...
    async saveImportedData(data) {
        ...
        if (!this.auth.canEditStandards())
            throw new Error('Bạn không có quyền nhập danh mục chuẩn.');
        ...
        for (let offset = 0; offset < validItems.length; offset += 20) {
            ...
            const snapshots = await Promise.all(chunk.map(item => getDoc(doc(this.fb.db, `artifacts/${this.fb.APP_ID}/reference_standards/${item.parsed.id}`))));
            snapshots.forEach(snapshot => {
                if (snapshot.exists())
                    existing.set(snapshot.id, { id: snapshot.id, ...snapshot.data() });
            });
        }
        const standardsCollection = collection(this.fb.db, `artifacts/${this.fb.APP_ID}/reference_standards`);
        const internalIds = [...new Set(validItems.map(item => item.parsed.internal_id?.trim()).filter((value) => Boolean(value)))];
        const activeByInternalId = new Map();
        for (let offset = 0; offset < internalIds.length; offset += 10) {
            const snapshot = await getDocs(query(standardsCollection, where('internal_id', 'in', internalIds.slice(offset, offset + 10))));
            snapshot.docs.forEach(document => {
                const standard = { id: document.id, ...document.data() };
                if (!isActiveStandardIdentity(standard))
                    return;
        ...
        const conflicts = validItems.filter(item => {
            ...
            if (!current)
                return false;
            return current.status === 'IN_USE' || Boolean(current.current_holder || current.current_holder_uid ||
        ...
        if (plannedWrites > STANDARD_IMPORT_MAX_ATOMIC_WRITES) {
            throw new Error(`Import cần ${plannedWrites} thao tác, vượt giới hạn an toàn ${STANDARD_IMPORT_MAX_ATOMIC_WRITES}. ` +
                'Vui lòng chia file thành các phần nhỏ hơn; chưa có dữ liệu nào được ghi.');
        }
        this.progressService.start('Đang lưu chất chuẩn đối chiếu', 'Toàn bộ dữ liệu sẽ được commit trong một giao dịch batch', validItems.length + 1);
        ...
        try {
            for (const item of validItems) {
                ...
                if (current) {
                    ...
                    const merged = { ...current, ...safeMetadata };
                ...
                for (const [logIndex, rawLog] of (item.logs || []).entries()) {
                    const amountToken = String(rawLog.normalized_amount ?? rawLog.amount_used)
                        .replace('-', 'neg_')
                        .replace('.', '_decimal_');
                    const signature = generateSlug(`${item.parsed.id}_${rawLog.date}_${rawLog.user}_${amountToken}_${rawLog.normalized_unit || item.parsed.unit}_${logIndex}`).slice(0, 180);
                    ...
                    batch.set(doc(this.fb.db, `artifacts/${this.fb.APP_ID}/standard_usages/${logId}`), { ...log, lastUpdated: serverTimestamp() });
                }
            }
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    const normalizeKey = (key) => key.toString().replace(/[\r\n]+/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
                    ...
                    for (const rawRow of rawRows) {
                        ...
                        else if (amountUsed <= 0 || normalizedAmount === null) {
                            isValid = false;
                            errorMessage = 'Lượng dùng hoặc đơn vị không hợp lệ.';
                        }
                        const log = {
                        ...
                        if (matchedStandard && isValid) {
                            if (!logsCache.has(matchedStandard.id)) {
                                ...
                                logsCache.set(matchedStandard.id, snap.docs.map(d => d.data()).filter(existingLog => !existingLog._isDeleted));
                            }
    ...
    async saveImportedUsageLogs(data) {
        if (!data || data.length === 0)
            return;
        if (!this.auth.canEditStandards())
            throw new Error('Bạn không có quyền nhập nhật ký sử dụng chuẩn.');
        const validItems = data.filter(item => item.isValid && !item.isDuplicate && item.standard);
        if (validItems.length === 0)
            return;
        const logsByStandard = new Map();
        for (const item of validItems) {
            ...
            for (const [stdId, { standard, logs }] of logsByStandard.entries()) {
                ...
                for (let offset = 0; offset < logs.length; offset += 100) {
                    ...
                    await runTransaction(this.fb.db, async (transaction) => {
                        ...
                        if (!stdDoc.exists())
                            throw new Error(`Chuẩn ${standard.name} không còn tồn tại.`);
                        const freshStandard = { id: stdDoc.id, ...stdDoc.data() };
                        if (freshStandard.status === 'IN_USE' || freshStandard.current_request_id ||
                        ...
                        if (newAmount < -1e-9) {
                            throw new Error(`Không đủ tồn kho cho ${freshStandard.name}: cần ${totalDeducted} ${freshStandard.unit}, ` +
                        ...
                        accepted.forEach(entry => {
                            ...
                            transaction.set(doc(this.fb.db, `artifacts/${this.fb.APP_ID}/standard_usages/${entry.log.id}`), { ...entry.log, lastUpdated: serverTimestamp() });
                        });
                    });
                }
            }
    ...
    static { this.ɵfac = function StandardImportService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || StandardImportService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: StandardImportService, factory: StandardImportService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(StandardImportService, [{
        type: Injectable,
        args: [{ providedIn: 'root' }]
    }], null, null); })();
//# sourceMappingURL=standard-import.service.js.map