...
export function mapCompoundToKey(c) {
    return getCanonicalId(c);
}
export function isDetectedValue(val) {
    if (val === undefined || val === null)
        return false;
    const s = String(val).trim().toUpperCase();
    return s !== '' && s !== 'ND' && s !== 'ND' && s !== 'N/A' && s !== '—';
}
export function buildTargetMetadata(compounds, masterTargets, sopIdOrConfigKey) {
    const targetInfo = {};
    if (!compounds)
        return targetInfo;
    compounds.forEach(c => {
...
export function buildTrifluralinPdfPayload(currentDraft, currentRun, activeFilter, formatAnalysisDate, getRunDate) {
    const sampleList = currentRun.sampleList || [];
    const checkedSamples = sampleList.filter((s) => {
    ...
    if (mappedMetadata.calibPoints && Array.isArray(mappedMetadata.calibPoints)) {
        mappedMetadata.calibPoints = mappedMetadata.calibPoints.map((pt) => ({
            ...pt,
            loSo_old: pt.loSo,
...
export function buildFipronilPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets = []) {
    const activeCols = Object.keys(currentConf.columns || {}).filter((c) => c !== 'loSo' && c !== 'maSoMau' && c !== 'ghiChu');
    const samplesPayload = [];
    const ensureKeyAndGet = (key, defaultVial, label) => {
...
export function buildDichlorvosPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const getQcRow = (key, label) => {
        const resObj = currentDraft.resultData[key] || {};
        const rowData = {
            loSo: resObj['loSo'] || '',
            maSoMau: label,
    ...
    const mappedMetadata = {
        ...
        ...(currentDraft.page1Data['calibPoints'] || []).reduce((acc, pt, index) => {
...
export function buildChloroformPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const getQcRow = (key, label) => {
        const resObj = currentDraft.resultData[key] || {};
    ...
    if (mappedMetadata.calibPoints && Array.isArray(mappedMetadata.calibPoints)) {
        mappedMetadata.calibPoints = mappedMetadata.calibPoints.map((pt, index) => {
            const flattenedPt = {
                ...pt,
                loSo_old: pt.loSo,
                loSo: pt.vialNo || pt.loSo,
...
export function buildDefaultSopPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const sampleList = currentRun.sampleList || [];
    const sampleTargetMap = currentRun.sampleTargetMap || (currentRun.inputs && currentRun.inputs.sampleTargetMap) || {};
    const isAssigned = (sampleCode, compound) => {
    ...
    if (isGop && filteredSamples.length > 0) {
        ...
        const isAssignedToAny = (compound) => {
            return filteredSamples.some((s) => isAssigned(s, compound));
        };
        currentConf.compounds.forEach((c) => {
            ...
            if (!assigned) {
                ...
                const hasDetection = filteredSamples.some((s) => isAssigned(s, c) &&
                ...
                const getMergedQc = (qcKey) => {
                    const assignedSamples = filteredSamples.filter((s) => isAssigned(s, c));
                    if (assignedSamples.length === 0)
                        return 'N/A';
                    const uniqueQcs = new Set(assignedSamples.map((s) => {
                        const sRes = currentDraft.resultData[s] || {};
                        return sRes[`${c}_${qcKey}`] || 'Đạt';
                    }));
                    if (uniqueQcs.has('Không đạt') || uniqueQcs.has('Không Đạt'))
                        return 'Không đạt';
                    if (uniqueQcs.has('Đạt'))
                        return 'Đạt';
                    return 'Đạt';
                };
        ...
        if (isDon && currentConf.formType === 'type3b') {
            ...
            if (currentDraft.page1Data['hasFinal'])
                formDonSamples.push('QC_FINAL');
        }
        formDonSamples.forEach((sampleCode, idx) => {
            const resObj = currentDraft.resultData[sampleCode] || {};
            if (currentConf.formType === 'type3b') {
                let displayMaSoMau = sampleCode;
                if (sampleCode === 'QC_BLANK')
                    displayMaSoMau = currentDraft.page1Data['blankName'] || 'Blank';
                else if (sampleCode === 'QC_SPIKE')
                    displayMaSoMau = currentDraft.page1Data['spikeName'] || 'Spike';
                else if (sampleCode === 'QC_FINAL')
                    displayMaSoMau = 'FINAL';
                const rowData = {
...
export function buildLanHuuCoPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets = []) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const sampleList = currentRun.sampleList || [];
    const filteredSamples = sampleList.filter((s) => {
    ...
    if (isGop && filteredSamples.length > 0) {
        ...
        formDonSamples.forEach((sampleCode) => {
            const resObj = currentDraft.resultData[sampleCode] || {};
            let displayMaSoMau = sampleCode;
            if (sampleCode === 'QC_BLANK')
                displayMaSoMau = currentDraft.page1Data['blankName'] || 'Blank';
    ...
    const buildCompoundMaps = (resKey) => {
    ...
    if (isGop && filteredSamples.length > 0) {
        ...
        const compoundResults = currentConf.compounds.reduce((acc, c) => {
            const backendKey = c;
        ...
        const compoundNotes = currentConf.compounds.reduce((acc, c) => {
            const backendKey = c;
            const notes = filteredSamples.map((sCode) => {
                const sRes = currentDraft.resultData[sCode] || {};
    ...
    const activeCompoundsConfig = isDon
        ...
        : currentConf.compounds.filter((c) => {
            return filteredSamples.some((s) => isAssigned(s, c));
        });
...
export function buildChlorHuuCoPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets = []) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const sampleList = currentRun.sampleList || [];
    const filteredSamples = sampleList.filter((s) => {
    ...
    if (isGop && filteredSamples.length > 0) {
        ...
        const compoundResults = currentConf.compounds.reduce((acc, c) => {
            ...
            const allKph = vals.every((v) => v === 'N/A' || v === '');
            const backendKey = c;
    ...
    const compoundsToPrint = activeCompoundsConfig.map((c) => mapCompoundToKey(c));
    return {
        action: 'generate_pdf',
        sopId: 'chlor-huu-co',
        metadata: {
...
export function buildNhomCucPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets = []) {
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const samplesPayload = [];
    const sampleList = currentRun.sampleList || [];
    const filteredSamples = sampleList.filter((s) => {
 ...
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
export function buildUnifiedType3bPdfPayload(currentDraft, currentRun, activeFilter, currentConf, formatAnalysisDate, getRunDate, masterTargets = []) {
    const sopId = currentConf.id || 'type3b';
    const prefixForReport = activeFilter === 'ALL' ? '' : activeFilter;
    const sampleList = currentRun.sampleList || [];
    const sampleTargetMap = currentRun.sampleTargetMap || currentRun.inputs?.sampleTargetMap || {};
    // ── Helpers ──────────────────────────────────────────────────────────────
    const filterSample = (s) => {
        const resObj = currentDraft.resultData[s] || {};
    ...
    const targetInfo = {};
    currentConf.compounds.forEach(canonicalId => {
     ...
     */
    const buildCompoundFields = (resObj, compound, sampleCode) => {
    ...
    const getDisplayLabel = (code) => {
        if (code === 'QC_BLANK')
            return currentDraft.page1Data['blankName'] || 'BLANK';
        if (code === 'QC_SPIKE')
            return currentDraft.page1Data['spikeName'] || 'SPIKE';
        if (code === 'QC_FINAL')
            return 'FINAL';
        return code;
    };
    if (isGop && filteredSamples.length > 0) {
        ...
        const isAssignedToAny = (compound) => filteredSamples.some(s => isAssigned(s, compound));
        const getMergedQc = (compound, qcKey) => {
            const assignedSamples = filteredSamples.filter(s => isAssigned(s, compound));
            if (assignedSamples.length === 0)
                return 'N/A';
            const vals = new Set(assignedSamples.map(s => {
                const sRes = currentDraft.resultData[s] || {};
                return sRes[`${compound}_${qcKey}`] || 'Đạt';
            }));
            if (vals.has('Không đạt') || vals.has('Không Đạt'))
                return 'Không đạt';
            if (vals.has('Đạt'))
                return 'Đạt';
            return 'Đạt';
    ...
    const buildRunRow = (key, label, loSoDefault) => {
        const resObj = currentDraft.resultData[key] || {};
    ...
    const compoundsToPrint = isDon
        ? [currentDraft.page1Data['activeCompound'] || currentConf.compounds[0]]
        : currentConf.compounds.filter(c => filteredSamples.some(s => isAssigned(s, c)));
    // ── Final payload ────────────────────────────────────────────────────────
    return {
        action: 'generate_pdf',
        sopId,
        metadata: {
            ...
            spikeName: currentDraft.page1Data['spikeName'] || 'SPIKE',
            hasFinal: currentDraft.page1Data['hasFinal'] === true,
            calibPoints: (currentDraft.page1Data['calibPoints'] || []).map((pt, index) => {
                return {
                    ...pt,
                    loSo_old: pt.loSo,
                    loSo: pt.vialNo || pt.loSo,
...
//# sourceMappingURL=result-pdf-helper.js.map