...
export class SopDraftFactoryService {
    createInitialDraft(runDoc, sopConf, options) {
        ...
        if (isTrifluralin) {
            ...
            Object.values(sopConf.checkboxLines).forEach((field) => {
                if (field === 'checkTatCaND' || field === 'checkCoMauPhatHien')
                    return;
                if (field === 'qcNhanDang')
                    defaultPage1[field] = null;
                else if (typeof field === 'string' && field.startsWith('qc'))
                    defaultPage1[field] = true;
                else
        ...
        if (isTrifluralin) {
            ...
            const createQcRow = (loSo) => {
                const row = { loSo, selected: true };
            ...
            sampleList.forEach((sampleCode, idx) => {
                ...
                if (sopConf.formType === 'type3b') {
                    ...
                    (sopConf.compounds || []).forEach((compound) => {
                    ...
                    Object.keys(sopConf.columns || {}).forEach(col => {
                        if (!['loSo', 'maSoMau', 'ghiChu'].includes(col))
                            row[col] = '';
                    });
                }
            });
    ...
    static { this.ɵfac = function SopDraftFactoryService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || SopDraftFactoryService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: SopDraftFactoryService, factory: SopDraftFactoryService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(SopDraftFactoryService, [{
        type: Injectable,
        args: [{ providedIn: 'root' }]
    }], null, null); })();
//# sourceMappingURL=sop-draft-factory.service.js.map