 ...
 */
import { Input, Output, EventEmitter, signal, computed, inject, Directive } from '@angular/core';
import { MasterTargetService } from '../../../targets/master-target.service';
import { resolveCompoundDisplayName, COMPOUND_TO_FIRESTORE_ID, getAssignedTargetsForSample, isCompoundAssigned } from '../../shared/compound-id-resolver';
 ...
 */
const LEGACY_DISPLAY_TO_CANONICAL = {};
...
export class AbstractSopEntry {
    constructor() {
        ...
        this.assignedCompoundsForFormDon = computed(() => {
            ...
            if (!targetMap)
                return compounds;
            return compounds.filter((c) => sampleList.some((s) => this.isTargetAssigned(s, c)));
        });
        // ── Target Assignment ─────────────────────────────────────────────────────
        /**
     ...
     */
    initCalibrationPoints(count, defaults) {
        const makePoints = (nongs) => nongs.map((hamLuong, idx) => ({ loSo: `C${idx}`, vialNo: String(idx + 1), hamLuong }));
    ...
    initActiveCompound() {
        ...
        let firstAssigned = targetMap
            ? compounds.find((c) => sampleList.some((s) => this.isTargetAssigned(s, c)))
            : compounds[0];
        if (!firstAssigned && compounds.length > 0)
     ...
     */
    migrateResultDataKeys() {
        if (!this.config?.compounds || !this.draft.resultData)
            return;
        const canonicalIds = this.config.compounds;
        ...
        for (const rowKey of allRowKeys) {
            ...
            for (const canonicalId of canonicalIds) {
                ...
                for (const rowDataKey of rowDataKeys) {
                    ...
                    if (mappedCanonical !== canonicalId)
                        continue;
    ...
    _computeIsTargetAssigned(sampleCode, compound, targetMap) {
    ...
    cleanUpPooledSampleResults() {
        if (!this.run?.sampleList || !this.config?.compounds)
            return;
    ...
    // ── Helper cho Mã Hồ Sơ ───────────────────────────────────────────────────
    autoFillMaHoSo() {
        const selectedSamples = (this.run?.sampleList || []).filter((sampleCode) => this.draft.resultData[sampleCode]?.['selected'] !== false);
    ...
    syncMassToSamples(oldMass, newMass) {
        if (oldMass === newMass || !newMass)
            return;
        (this.run?.sampleList || []).forEach((sampleCode) => {
            const row = this.draft.resultData[sampleCode];
            if (row) {
    ...
    setCompoundQc(compound, qcKey, status) {
        const active = this.activeSampleCode();
        const row = this.draft.resultData[active];
        if (row && this.isTargetAssigned(active, compound)) {
    ...
    // ── Sample Selection ──────────────────────────────────────────────────────
    getSelectedSampleCount() {
        return (this.run?.sampleList || [])
            .filter((s) => this.draft.resultData[s]?.['selected'] !== false).length;
    }
    ...
    toggleSampleSelected(sampleCode, checked) {
        if (!this.draft.resultData[sampleCode]) {
            this.draft.resultData[sampleCode] = {};
        }
    ...
    isAllSamplesSelected() {
        const sampleList = this.run?.sampleList || [];
        if (sampleList.length === 0)
            return false;
        return sampleList.every((s) => this.draft.resultData[s]?.['selected'] !== false);
    }
    toggleSelectAllSamples() {
        const targetState = !this.isAllSamplesSelected();
        (this.run?.sampleList || []).forEach((s) => {
    ...
    setPrintFormType(type) {
        ...
        if (type === 'formDon') {
            ...
            if (activeCompound)
                this.draft.page1Data['r2'] = byCompound[activeCompound];
            const calPoints = this.draft.page1Data['calibPoints'];
            if (calPoints && calPoints.length > 0) {
                this.bulkCalibVialStart = parseInt(calPoints[0].loSo, 10) || 1;
            }
    ...
    getChromatographyRows() {
        ...
        (this.run?.sampleList || []).forEach((sampleCode, idx) => {
            if (!this.draft.resultData[sampleCode]) {
                ...
                if (!sRow['loSo'])
                    sRow['loSo'] = (9 + idx).toString();
                if (!sRow['heSoPhaLoang'])
                    sRow['heSoPhaLoang'] = '1';
                if (!sRow['hSoPhaLoang'])
                    sRow['hSoPhaLoang'] = '1';
    ...
    bulkFillNDFormDon() {
        ...
        if (!active)
            return;
        this.getChromatographyRows().forEach((row) => {
    ...
    static { this.ɵfac = function AbstractSopEntry_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || AbstractSopEntry)(); }; }
    static { this.ɵdir = /*@__PURE__*/ i0.ɵɵdefineDirective({ type: AbstractSopEntry, inputs: { run: "run", draft: "draft", config: "config", isReadOnly: "isReadOnly", publishedSampleSet: "publishedSampleSet" }, outputs: { draftChanged: "draftChanged" }, features: [i0.ɵɵNgOnChangesFeature] }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(AbstractSopEntry, [{
        type: Directive
    }], null, { run: [{
            type: Input
        }], draft: [{
...
//# sourceMappingURL=abstract-sop-entry.js.map