export function navigateGrid(event, rowIdx, colIdx, columnsList, rowsLength, minColIdx = 0) {
    const key = event.key;
...
export function bulkFillND(resultData, sampleList, activeColumns, onUpdateRecovery) {
    if (!resultData)
        return;
    // Điền các mẫu thường
...
export function bulkClearAll(resultData, sampleList, activeColumns) {
    if (!resultData)
        return;
    (sampleList || []).forEach((sampleCode) => {
        const row = resultData[sampleCode];
...
export function copyRowToAll(resultData, sampleList, activeColumns, sourceKey, onUpdateRecovery) {
    if (!resultData)
        return;
...
//# sourceMappingURL=sop-grid-helper.js.map