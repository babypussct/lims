 ...
 */
export const COMPOUND_TO_FIRESTORE_ID = {
    // BHC isomers
 ...
 */
export const SOP01_COLUMN_TO_CANONICAL = {
    'kqFip': 'fipronil',
 ...
 */
export function getSop01DisplayName(colKey, masterTargets) {
    ...
    if (masterTargets && masterTargets.length > 0) {
        ...
        const found = masterTargets.find(t => getCanonicalId(t.id) === canonicalSearch ||
...
export function resolveCompoundDisplayName(compound, analytes, sopIdOrConfigKey) {
    ...
    const exactMatch = analytes.find(a => a.id.toLowerCase() === compound.toLowerCase() ||
    ...
    if (exactMatch) {
        ...
        const canonicalMatch = analytes.find(a => getCanonicalId(a.id) === canonicalCompound ||
...
let firestoreIdsCache = null;
export function getCanonicalId(name) {
    if (!name)
        return '';
 ...
 */
const normalizedSampleTargetMapCache = new WeakMap();
export function getAssignedTargetsForSample(sampleCode, sampleTargetMap) {
    if (!sampleCode || !sampleTargetMap)
        return null;
...
export function normalizeSampleCode(sampleCode) {
 ...
 */
export function isCompoundAssigned(assignedTargetIds, compound, masterTargets) {
    if (!assignedTargetIds || assignedTargetIds.length === 0)
        return true;
    if (!compound)
        return false;
    const canonicalCompound = getCanonicalId(compound);
    return assignedTargetIds.some(tId => {
        // Direct string match
        if (tId.toLowerCase() === compound.toLowerCase())
            return true;
        let targetName = tId;
        if (masterTargets && masterTargets.length > 0) {
            ...
            const directMatch = masterTargets.find((t) => t.id === tId);
            const found = directMatch ?? resolveTargetMasterInfo(tId, masterTargets);
            if (found && found.name) {
                targetName = found.name;
                if (targetName.toLowerCase() === compound.toLowerCase())
                    return true;
                if (getCanonicalId(found.name) === canonicalCompound)
                    return true;
            }
        }
 ...
 */
export function resolveTargetMasterInfo(compound, masterTargets) {
    if (!compound || !masterTargets || masterTargets.length === 0)
        return null;
    const targetIdStr = getCanonicalId(compound);
    // 1. Match by exact ID or Canonical ID
    let match = masterTargets.find((t) => t.id === targetIdStr ||
...
//# sourceMappingURL=compound-id-resolver.js.map