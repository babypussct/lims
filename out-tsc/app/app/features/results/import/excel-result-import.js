import { COMPOUND_TO_FIRESTORE_ID, SOP01_COLUMN_TO_CANONICAL, getAssignedTargetsForSample, getCanonicalId, isCompoundAssigned } from '../shared/compound-id-resolver';
...
export function parseMassHunterResultWorkbook(XLSX, workbook) {
    const compounds = [];
    const warnings = [];
...
export function buildExcelImportCandidates(parsed, context) {
    ...
    parsed.compounds.forEach((compound, compoundIndex) => {
        ...
        compound.rows.forEach((row, rowIndex) => {
            candidates.push(buildResultCandidate(row, target, context, `result-${compoundIndex}-${rowIndex}`));
        });
    ...
    if (isFormDon) {
        ...
        if (pointNames.length > 0) {
            ...
            candidates.push({
                ...
                currentValue: existing.map((point, index) => point?.loSo || `C${index}`).join(', '),
...
export function getRelevantExcelImportSheetNames(sheetNames, context) {
    const targets = buildResultTargets(context.config, buildTargetResolutionCatalog(context));
    return sheetNames.filter(sheetName => Boolean(matchCompoundTarget(sheetName, targets)));
}
export function updateCandidateSample(candidate, targetSample, context) {
    ...
    if (!isTargetAllowedForSample(targetSample, candidate.compoundId, context.run, buildTargetResolutionCatalog(context))) {
...
export function applyExcelImportCandidates(candidates, context, fileName, decimalPlaces = null, writeNdToResult = !usesNdCheckbox(context)) {
    ...
    for (const candidate of selected) {
        if (candidate.kind === 'result') {
            if (!candidate.targetSample || !candidate.targetField)
                continue;
    ...
    if (auditTrail.length > 20)
        auditTrail.splice(0, auditTrail.length - 20);
    context.draft.page1Data['lastExcelResultImport'] = auditTrail[auditTrail.length - 1];
    return applied;
}
export function normalizeImportSampleName(value) {
    return String(value || '')
        .trim()
        .toUpperCase()
        .replace(/\.D$/i, '')
...
export function getAvailableExcelImportSamples(context) {
...
export function formatImportedFinalConc(sourceValue, isNd, decimalPlaces) {
    if (isNd || isNdValue(sourceValue))
        return 'ND';
...
function findHeaderRow(XLSX, sheet, range) {
    const lastHeaderRow = Math.min(range.e.r, range.s.r + 24);
    for (let row = range.s.r; row <= lastHeaderRow; row++) {
        let sampleColumn = null;
        let finalConcColumn = null;
        let typeColumn = null;
        for (let column = range.s.c; column <= range.e.c; column++) {
            const cell = sheet[XLSX.utils.encode_cell({ r: row, c: column })];
            const normalized = normalizeHeader(cellDisplayValue(cell));
            if (SAMPLE_HEADER_NAMES.has(normalized))
                sampleColumn = column;
            if (FINAL_CONC_HEADER_NAMES.has(normalized))
                finalConcColumn = column;
            if (TYPE_HEADER_NAMES.has(normalized))
                typeColumn = column;
        }
...
function buildResultCandidate(row, target, context, id) {
    const regularSamples = (context.run?.sampleList || []).map(String);
    ...
    if (!isTargetAllowedForSample(targetSample, target.compoundId, context.run, buildTargetResolutionCatalog(context))) {
        return {
            id,
            kind: 'result',
            sheetName: row.sheetName,
    ...
    const current = readResultCurrentValue(context.draft?.resultData?.[targetSample] || {}, target.resultKey, usesNdCheckbox(context));
    return {
        id,
        kind: 'result',
        sheetName: row.sheetName,
...
function buildResultTargets(config, masterTargets) {
    if (config?.formType === 'type3b') {
        return (config.compounds || []).map((compoundId) => ({
            compoundId,
    ...
    return Object.keys(config?.columns || {})
        ...
        return {
            compoundId,
            resultKey,
            aliases: aliasesForCompound(compoundId, resultKey.replace(/^kq/i, ''), masterTargets)
        };
    });
}
/**
 ...
 */
function buildTargetResolutionCatalog(context) {
    ...
    snapshots.forEach(snapshot => {
        ...
        Object.entries(snapshot).forEach(([id, name]) => {
            ...
            const aliases = new Set(Array.isArray(existing.aliases) ? existing.aliases.map(String) : []);
...
function aliasesForCompound(compoundId, extraAlias = '', masterTargets = []) {
    ...
    for (const [alias, canonical] of Object.entries(COMPOUND_TO_FIRESTORE_ID)) {
        if (canonical === compoundId)
            aliases.add(normalizeCompoundName(alias));
    }
    const canonicalId = getCanonicalId(compoundId);
    masterTargets
        .filter(target => getCanonicalId(String(target?.id || '')) === canonicalId
        ...
        (Array.isArray(target.aliases) ? target.aliases : []).forEach((alias) => {
...
function matchCompoundTarget(sourceName, targets) {
    ...
    const contained = targets.filter(target => Array.from(target.aliases).some(alias => alias.length >= 5 && (normalized.includes(alias) || alias.includes(normalized))));
    return contained.length === 1 ? contained[0] : null;
}
function matchRegularSamples(excelSampleName, samples) {
    ...
    if (suffixMatches.length > 1) {
        const longestLength = Math.max(...suffixMatches.map(sample => normalizeImportSampleName(sample).length));
        return suffixMatches.filter(sample => normalizeImportSampleName(sample).length === longestLength);
    }
    ...
    return samples.filter(sample => normalizeImportSampleName(sample) === limsSampleName);
}
function classifyQcSample(sampleName, sampleType, context) {
    ...
    if (isSop01Import(context)) {
        ...
        if (numberedSpike)
            return `QC_SPIKE_${Number(numberedSpike[1])}`;
        ...
        if (/(?:^|_)(?:BLANK|BL)[A-Z0-9]*(?:_|$)/.test(normalized))
            return 'QC_BLANK';
        if (/(?:^|_)(?:SPIKE|SP)[A-Z0-9]*(?:_|$)/.test(normalized))
            return 'QC_SPIKE';
        const checkSampleName = normalizeImportSampleName(String(context.draft?.page1Data?.['checkSampleName'] || ''));
        if (checkSampleName && normalized.includes(checkSampleName))
            return 'QC_CHECK_SAMPLE';
    }
    if (lastToken === 'BL' || lastToken === 'BLANK')
        return 'QC_BLANK';
    if (lastToken === 'SP' || lastToken === 'SPIKE')
        return 'QC_SPIKE';
    if (lastToken === 'FINAL')
        return 'QC_FINAL';
    if (lastToken === 'CHECK' || normalized.endsWith('_CHECK_SAMPLE'))
        return 'QC_CHECK_SAMPLE';
    return undefined;
}
function isSop01Import(context) {
    return context.configKey === 'fipronil-chlorpyrifos'
        || context.run?.sopId === 'SOP-01';
}
function compareDynamicSpikeSamples(left, right) {
    return Number(left.replace('QC_SPIKE_', '')) - Number(right.replace('QC_SPIKE_', ''));
}
function isTargetAllowedForSample(sample, compoundId, run, masterTargets = []) {
    if (sample.startsWith('QC_'))
        return true;
...
function isQcTargetAvailable(sample, context) {
    if (!sample.startsWith('QC_'))
        return true;
    ...
    if (context.config?.formType === 'type3b') {
        ...
        if (printFormType === 'formDon') {
            if (sample === 'QC_FINAL')
                return context.draft?.page1Data?.['hasFinal'] === true;
            if (sample === 'QC_CHECK_SAMPLE')
                return false;
...
function usesNdCheckbox(context) {
    return context.config?.formType === 'type3b'
        && context.draft?.page1Data?.['printFormType'] === 'formCheck';
}
function readResultCurrentValue(row, resultKey, ndCheckbox) {
    if (ndCheckbox && row?.[`${resultKey}_nd`] === true)
        return 'ND';
    return String(row?.[resultKey] ?? '').trim();
}
function readCurrentR2(context, compoundId) {
    if (context.config?.formType !== 'type3b') {
        return String(context.draft?.page1Data?.['r2'] ?? '').trim();
    }
    const byCompound = context.draft?.page1Data?.['r2ByCompound'] || {};
    if (byCompound[compoundId] !== undefined)
        return String(byCompound[compoundId]).trim();
    if (context.draft?.page1Data?.['activeCompound'] === compoundId) {
        return String(context.draft?.page1Data?.['r2'] ?? '').trim();
    }
    return '';
}
function applyCalibrationPointNames(page1Data, pointNames) {
    ...
    page1Data['calibPoints'] = pointNames.map((name, index) => {
        const sameName = existing.find((point) => normalizeImportSampleName(point?.loSo) === normalizeImportSampleName(name));
...
function chooseCalibrationPointNames(compounds) {
    const groups = new Map();
    compounds.forEach(compound => {
        if (compound.calibrationPointNames.length === 0)
            return;
        ...
        if (current)
            ...
            groups.set(key, { names: compound.calibrationPointNames, count: 1 });
    });
    return Array.from(groups.values()).sort((a, b) => b.count - a.count)[0]?.names || [];
}
function isCalibrationRow(sampleName, sampleType) {
    const type = normalizeImportSampleName(sampleType);
    if (type.includes('CALIBRATION'))
        return true;
    return extractCalibrationPointName(sampleName) !== null;
}
function extractCalibrationPointName(sampleName) {
    const normalized = normalizeImportSampleName(sampleName);
    const match = normalized.match(/(?:^|_)(C\d+)$/);
    return match?.[1] || null;
}
function compareCalibrationPoints(a, b) {
    return Number(a.slice(1)) - Number(b.slice(1));
}
function normalizedResultValue(row) {
...
//# sourceMappingURL=excel-result-import.js.map