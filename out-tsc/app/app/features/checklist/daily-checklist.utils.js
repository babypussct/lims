...
import { buildTargetScopePresentation, classifyTargetScope, computeTargetSignature } from '../targets/target-scope-classifier';
const APPROVED_BATCH_STATUSES = new Set(['approved', 'draft', 'completed']);
...
export function getRequestDateValue(request) {
    return isValidDateInput(request.analysisDate || '') ? request.analysisDate : '';
}
export function buildApprovedBatchOverviews(requests, selectedDate, resolveTargetName) {
    ...
    return Array.from(uniqueRequests.values())
        ...
        const samples = uniqueSampleCodes(request.sampleList || []).map(sampleId => {
...
export function buildDailyBatchViews(batches, availableGroups = []) {
    const sopCards = new Map();
    ...
    return Array.from(sopCards.entries()).map(([cardKey, sourceBatches]) => {
        const sortedSources = [...sourceBatches].sort((a, b) => (b.approvedAt?.getTime() || 0) - (a.approvedAt?.getTime() || 0)
            || naturalCompare(a.requestId, b.requestId));
        const representative = sortedSources[0];
        const targetSetMap = new Map();
        const groupSampleMaps = new Map();
        const allSampleKeys = new Set();
        const allTargetIds = new Set();
        const ensureTargetGroup = (batch, targetIds, targetNames) => {
        ...
        const groups = Array.from(targetSetMap.values()).map(group => {
            ...
            if (a.targetScope.kind === 'unassigned')
                return 1;
            if (b.targetScope.kind === 'unassigned')
                return -1;
...
export function isTrackablePhysicalBatch(request) {
...
function compareDailySampleIds(a, b) {
...
//# sourceMappingURL=daily-checklist.utils.js.map