...
export function planDailyPrintLayout(batches, groupSamples, orientationPreference = 'auto', modePreference = 'auto') {
    const candidates = ['compact', 'list'].flatMap(mode => ['portrait', 'landscape']
        .map(orientation => evaluateLayout(batches, groupSamples, mode, orientation)));
    const eligible = candidates.filter(candidate => (orientationPreference === 'auto' || candidate.orientation === orientationPreference)
...
function evaluateLayout(batches, groupSamples, mode, orientation) {
    return mode === 'compact'
        ? evaluateCompactLayout(batches, groupSamples, orientation)
        : evaluateListLayout(batches, groupSamples, orientation);
}
function evaluateListLayout(batches, groupSamples, orientation) {
    ...
    batches.forEach(batch => {
        ...
        batch.groups.forEach(group => {
            ...
            const lines = Math.max(estimateLines(batchText, metrics.batchCharsPerLine), estimateLines(sampleText, metrics.sampleCharsPerLine), estimateLines(targetText, metrics.targetCharsPerLine), 1);
...
function evaluateCompactLayout(batches, groupSamples, orientation) {
...
export function buildDailyCompactPrintPages(batches, groupSamples, orientation) {
    return buildCompactPlacement(batches, groupSamples, orientation).pages;
}
function buildCompactPlacement(batches, groupSamples, orientation) {
    if (batches.length === 0) {
    ...
    let wrappedLineCount = 0;
    const startPage = () => {
        currentColumns = Array.from({ length: metrics.compactColumns }, () => []);
...
function estimateCompactCard(batch, groupSamples, metrics) {
...
function buildReason(selected, orientationPreference, modePreference) {
...
//# sourceMappingURL=daily-print-layout-planner.js.map