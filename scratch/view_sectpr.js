...
function printSectPr(c) {
    const match = c.xml.match(/<w:sectPr[\s\S]*?<\/w:sectPr>/) || c.xml.match(/<w:sectPr[^>]*\/>/);
    if (match) {
        console.log(`--- SectPr in Idx ${c.index} ---`);
        console.log(match[0]);
    }
}
...
children.forEach(c => {
    if (c.xml.includes('<w:sectPr')) {
        console.log(`Child index ${c.index} (tag: ${c.tag}) has sectPr`);
        printSectPr(c);
    }
});
