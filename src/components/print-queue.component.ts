            ...
            <div class="flex-1 overflow-y-auto">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @for (log of state.printableLogs(); track log.id) {
...
export class PrintQueueComponent {
  ...
  areAllSelected = computed(() => {
    const numPrintable = this.state.printableLogs().length;
    return numPrintable > 0 && this.selectedLogIds().size === numPrintable;
  });
  ...
  toggleSelectAll() {
    const allIds = this.state.printableLogs().map(log => log.id);
    if (this.areAllSelected()) {
      this.selectedLogIds.set(new Set());
    } else {
      this.selectedLogIds.set(new Set(allIds));
    }
  }
  ...
  async printSingle(log: Log) {
    ...
    // Explicitly mapping log.user to PrintJob.user to fix "Unknown" issue
  ...
  async printSelected() {
    ...
    const logsToPrint = this.state.printableLogs().filter(log => ids.has(log.id));
    
    const jobs: PrintJob[] = logsToPrint.map(log => ({
        ...log.printData!,
        date: log.timestamp.toDate(), // Use original approval date
        user: log.user // Fix Unknown user
    }));
