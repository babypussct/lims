    ...
    currentConf.compounds.forEach((c: string) => {
      ...
      if (!assigned) {
        ...
        const getMergedQc = (qcKey: string): string => {
          ...
          if (assignedSamples.length === 0) return 'N/A';
          const uniqueQcs = new Set<string>(assignedSamples.map((s: string) => {
            const sRes = currentDraft.resultData[s] || {};
            return (sRes[${c}_] as string) || 'Äáº¡t';
          }));
          if (uniqueQcs.has('KhÃ´ng Ä‘áº¡t') || uniqueQcs.has('KhÃ´ng Äáº¡t')) return 'KhÃ´ng Ä‘áº¡t';
          if (uniqueQcs.has('Äáº¡t')) return 'Äáº¡t';
          return 'Äáº¡t';
        };
  if (!compounds) return targetInfo;
  compounds.forEach(c => {
...
export function buildUnifiedType3bPdfPayload(
  ...
  if (isGop && filteredSamples.length > 0) {
    ...
    const getMergedQc = (compound: string, qcKey: string): string => {
      ...
      if (assignedSamples.length === 0) return 'N/A';
      const vals = new Set<string>(assignedSamples.map(s => {
        const sRes = currentDraft.resultData[s] || {};
        return (sRes[`${compound}_${qcKey}`] as string) || 'Đạt';
      }));
      if (vals.has('Không đạt') || vals.has('Không Đạt')) return 'Không đạt';
      if (vals.has('Đạt')) return 'Đạt';
      return 'Đạt';
    };
