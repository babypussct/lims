...
export interface ExcelImportReadProgress {
  stage: ExcelImportReadStage;
  percent: number;
  message: string;
}
...
export async function readExcelResultFile(
  file: File,
  ...
  if (typeof Worker !== 'undefined') {
    try {
      ...
      if (isAbortError(error) || !(error instanceof WorkerUnavailableError)) throw error;
...
function readInWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      worker = new Worker(
        new URL('./excel-result-import.worker', import.meta.url),
        { type: 'module' }
      );
    } catch (error) {
