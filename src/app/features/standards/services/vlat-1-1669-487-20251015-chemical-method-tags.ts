 ...
 */
const METHOD_DEVICE_OVERRIDES: Readonly<Record<string, readonly StandardDeviceCode[]>> = {
  'NAFI6/H-1.8': ['UVVIS'],
  'NAFI6/H-1.26': ['IC'],
  'NAFI6/H-2.2': ['UVVIS'],
  'NAFI6/H-2.15': ['UVVIS'],
...
function getMethodDeviceCodes(methodCode: string): StandardDeviceCode[] {
  const explicit = METHOD_DEVICE_OVERRIDES[methodCode];
  if (explicit) return [...explicit];

  const series = deriveMethodSeries(methodCode);
  if (series === 'H-6') return ['ICPMS'];
  if (series === 'H-7') return ['HPLC'];
  if (series === 'H-8') return ['LCMSMS'];
  return [];
}
