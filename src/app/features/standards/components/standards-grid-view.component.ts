                   ...
                   @for (std of items(); track std.id) {
                       <div class="bg-white dark:bg-slate-800 rounded-2xl border transition-all duration-200 flex flex-col relative group h-full hover:-translate-y-1 hover:shadow-lg dark:hover:shadow-none overflow-hidden"
                            [ngClass]="{
                                ...
                                'border-fuchsia-400 dark:border-fuchsia-500 shadow-md bg-fuchsia-50 dark:bg-fuchsia-900/30': !isAuditMode() && selectedIds().has(std.id!),
                                'opacity-50 grayscale hover:opacity-100 hover:grayscale-0': !isAuditMode() && (std.status === 'DEPLETED' || std.current_amount <= 0)
                            }">
                           ...
                           <div class="p-4 flex flex-col h-full">
                               ...
                               <div class="flex justify-between items-start mb-3">
                                   ...
                                   <input type="checkbox" [checked]="selectedIds().has(std.id!)" (change)="toggleSelection.emit(std.id!)" class="w-5 h-5 accent-fuchsia-600 dark:accent-fuchsia-500 cursor-pointer shrink-0 mt-0.5" [class.hidden]="isAuditMode()">
                               </div>
                               ...
                               <button type="button" class="mb-4 w-full cursor-pointer text-left group/identity" (click)="navigateToDetail.emit(std)">
                                   ...
                                   @if(!isAuditMode() && std.chemical_name) { <p class="text-xs text-slate-500 dark:text-slate-400 italic font-medium break-words"><span class="font-bold mr-1 text-slate-400">Tên đồng nghĩa:</span>{{std.chemical_name}}</p> }
                               </button>
                               ...
                               <div class="mt-auto">
                                   @if (isAuditMode()) {
                                       <div class="flex justify-between items-end mb-3">
                                           <div>
                                               <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase">Khối lượng chai</span>
                                               <div class="font-black text-fuchsia-600 dark:text-fuchsia-400 text-lg leading-none">{{formatNum(std.initial_amount)}} <small class="text-xs font-bold text-slate-400 dark:text-slate-500">{{std.unit || '—'}}</small></div>
                                           </div>
                                       </div>
                                       <div class="flex flex-col gap-1 min-h-[22px]">
                                           <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase">Điều kiện bảo quản</span>
