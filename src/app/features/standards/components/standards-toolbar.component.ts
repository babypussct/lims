         ...
         @if(canEditStandards() || state.showLockedFeatures()) {
            <div class="relative ml-1">
                <button
                    ...
                    class="h-10 px-3 bg-fuchsia-600 dark:bg-fuchsia-500 text-white hover:bg-fuchsia-700 dark:hover:bg-fuchsia-600 rounded-xl shadow-sm shadow-fuchsia-200 dark:shadow-none transition font-bold text-sm flex items-center gap-2 md:h-9">
                ...
                @if (functionMenuOpen()) {
                  <div id="standards-function-menu" role="menu" class="absolute right-0 top-full mt-1 w-56 max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 z-30 overflow-hidden flex flex-col p-1 animate-slide-up">
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="runMenuAction(openAddModal)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-fuchsia-50 hover:text-fuchsia-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-plus text-fuchsia-500 w-4"></i> Thêm mới
                    </button>
                    <div class="h-px bg-slate-100 dark:bg-slate-700 my-1 mx-2"></div>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="runMenuAction(openInternalIdSync)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 hover:text-amber-700 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-arrows-rotate text-amber-500 w-4"></i> Đồng bộ mã nội bộ
                    </button>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="openFilePicker(fileInput)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-file-excel text-emerald-500 w-4"></i> Nhập danh mục chuẩn
                    </button>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="openFilePicker(usageLogFileInput)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-teal-50 hover:text-teal-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-book-open text-teal-500 w-4"></i> Nhập nhật ký
                    </button>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="runMenuAction(openCleanupModal)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-fuchsia-50 hover:text-fuchsia-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-broom text-fuchsia-500 w-4"></i> Chuẩn hóa tên chất chuẩn
                    </button>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="runMenuAction(openTagManager)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-fuchsia-50 hover:text-fuchsia-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                    ...
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="openFilePicker(bulkCoaFolderInput)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
                        <i class="fa-solid fa-folder-open text-amber-500 w-4 ml-2"></i> Từ thư mục
                    </button>
                    <button role="menuitem" [appLockPermission]="'standard_edit'" (click)="openFilePicker(bulkCoaFilesInput)" class="min-h-10 text-left px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-700 rounded-lg transition flex items-center gap-2 md:min-h-0">
