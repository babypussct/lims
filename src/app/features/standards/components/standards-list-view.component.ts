    ...
    <div class="min-w-[1000px]" role="region" aria-label="Bảng danh sách chuẩn">
       <table class="w-full text-sm text-left relative border-collapse">
          <thead class="text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase bg-slate-50 dark:bg-slate-800/80 sticky top-0 z-10 border-b border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none h-12 tracking-wide">
             <tr>
                <th class="px-4 py-3 w-10 text-center" [class.hidden]="isAuditMode()"><input type="checkbox" [checked]="isAllSelected()" (change)="toggleAll.emit()" class="w-4 h-4 accent-fuchsia-600 dark:accent-fuchsia-500 cursor-pointer"></th>
                <th class="px-4 py-3 w-[25%]">{{ isAuditMode() ? 'Định danh' : 'Định danh & Vị trí' }}</th>
                <th class="px-4 py-3 w-[20%]">Thông tin lô/sản xuất</th>
                <th class="px-4 py-3 w-[15%]">{{ isAuditMode() ? 'Khối lượng chai & Bảo quản' : 'Tồn kho & Bảo quản' }}</th>
                <th class="px-4 py-3 w-[15%]">Hạn dùng & Hồ sơ</th>
                <th class="px-4 py-3 w-[10%] text-center" [class.hidden]="isAuditMode()">Trạng thái</th>
                <th class="px-4 py-3 w-[10%] text-center" [class.hidden]="isAuditMode()">Tác vụ</th>
             </tr>
          </thead>
          <tbody class="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
             @if (isLoading() && allStandardsLength() === 0) {
                  @for (i of [1,2,3,4,5]; track i) {
                      <tr class="h-24">
                          <td class="px-4" [class.hidden]="isAuditMode()"><app-skeleton width="16px" height="16px"></app-skeleton></td>
                          ...
                          <td class="px-4 text-center" [class.hidden]="isAuditMode()"><app-skeleton width="80px" height="24px" class="mx-auto rounded-full"></app-skeleton></td>
                          <td class="px-4 text-center" [class.hidden]="isAuditMode()"><app-skeleton width="60px" height="24px" class="mx-auto"></app-skeleton></td>
                      </tr>
                  }
               } @else {
                 @for (std of items(); track std.id) {
                    <tr class="hover:bg-fuchsia-50/30 dark:hover:bg-fuchsia-900/20 transition group h-24" [ngClass]="{'bg-fuchsia-50 dark:bg-fuchsia-900/30': !isAuditMode() && selectedIds().has(std.id!), 'opacity-50 grayscale hover:opacity-100 hover:grayscale-0': !isAuditMode() && (std.status === 'DEPLETED' || std.current_amount <= 0)}">
                       <td class="px-4 py-3 text-center align-top pt-4" [class.hidden]="isAuditMode()">
                       ...
                       <td class="px-4 py-3 align-top">
                          <div class="flex flex-col h-full">
                              ...
                              @if(!isAuditMode() && std.chemical_name) { <div class="text-xs text-slate-500 dark:text-slate-400 italic mb-2 break-words" [title]="std.chemical_name"><span class="font-bold mr-1 text-slate-400">Synonyms:</span>{{std.chemical_name}}</div> }
                       ...
                       <td class="px-4 py-3 align-top border-l border-slate-50 dark:border-slate-800">
                          ...
                          <div class="mt-2 pt-1 border-t border-slate-100 dark:border-slate-800 text-[10px] flex items-center gap-2 text-slate-500 dark:text-slate-400">
                              ...
                              @if(!isAuditMode() && std.pack_size) { <span>Quy cách: <b class="text-slate-700 dark:text-slate-300">{{std.pack_size}}</b></span> }
                          </div>
                       </td>
                       <td class="px-4 py-3 align-top border-l border-slate-50 dark:border-slate-800">
                          @if (isAuditMode()) {
                              <div class="flex flex-col gap-1">
                                  <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Khối lượng chai</span>
                                  <span class="text-lg font-black text-emerald-600 dark:text-emerald-400 leading-none">{{formatNum(std.initial_amount)}} <small class="text-[10px] font-bold text-slate-400 dark:text-slate-500">{{std.unit || '—'}}</small></span>
                              </div>
                              <div class="mt-3 flex flex-col gap-1">
                                  <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Điều kiện bảo quản</span>
                                  <span class="text-xs font-bold text-slate-700 dark:text-slate-300 break-words">{{std.storage_condition || '-'}}</span>
                              </div>
                          } @else {
                       ...
                       <td class="px-4 py-3 align-top border-l border-slate-50 dark:border-slate-800">
                          ...
                          <div class="flex flex-col gap-1.5">
                              ...
                              @else if(!isAuditMode() && currentUser()?.role === 'manager') { <button (click)="triggerQuickDriveUpload.emit({std: std, event: $event})" [disabled]="quickUploadStdId() === std.id" class="flex items-center gap-1.5 text-[10px] font-bold text-amber-600 dark:amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded border border-amber-200 dark:border-amber-800/50 hover:bg-amber-100 dark:hover:bg-amber-900/40 transition w-fit" title="Tải nhanh CoA lên Google Drive">
                              ...
                              @if(!isAuditMode() && std.contract_ref) { <div class="text-[10px] text-slate-400 dark:text-slate-500 truncate max-w-[120px] flex items-center gap-1" title="Hợp đồng"><i class="fa-solid fa-file-contract"></i> {{std.contract_ref}}</div> }
                          </div>
                       </td>
                       <td class="px-4 py-3 align-top text-center border-l border-slate-50 dark:border-slate-800" [class.hidden]="isAuditMode()">
