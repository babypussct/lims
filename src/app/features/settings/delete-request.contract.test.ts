...
describe('account anonymization contract', () => {
  it('does not modify Firestore when Firebase Auth anonymization fails', () => {
    const authUpdateBlock = source.match(/try \{\s*await auth\.updateUser[\s\S]*?\r?\n {4}\} catch \(authErr: any\) \{[\s\S]*?\r?\n {4}\}/)?.[0] || '';
  ...
  it('does not report a successful anonymization after a partial failure', () => {
    const failureSection = source.match(/\} catch \(firestoreErr: any\) \{[\s\S]*?\r?\n {4}\}\r?\n\r?\n {4}console\.log/)?.[0] || '';
