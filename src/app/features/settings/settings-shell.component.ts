...
@Component({
  ...
  template: `
    <div class="mx-auto w-full max-w-[1500px] px-3 pb-24 sm:px-5 lg:px-8">
      <app-page-header
        title="Cài đặt"
        subtitle="Quản lý tài khoản, hệ thống, dữ liệu và quyền truy cập."
        icon="fa-sliders">
        <div pageHeaderActions class="flex items-center gap-2">
          <span class="inline-flex h-9 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
            {{ auth.currentUser()?.role === 'manager' ? 'Quản trị viên' : 'Tài khoản' }}
          </span>
      ...
      <div class="mt-5 grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside class="lg:sticky lg:top-20 lg:self-start">
          <div class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:shadow-none">
            ...
            <div class="relative mb-3">
              ...
              <input
                ...
                class="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm font-medium text-slate-700 outline-none transition focus:border-indigo-400 focus:bg-white dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:focus:border-indigo-500"
              >
            </div>

            <nav aria-label="Điều hướng cài đặt" class="max-h-[calc(100vh-12rem)] space-y-4 overflow-y-auto custom-scrollbar">
              @for (group of filteredGroups(); track group.label) {
                <div>
                  <div class="mb-1 px-2 text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">{{ group.label }}</div>
                  <div class="space-y-1">
                    @for (item of group.items; track item.path) {
                      <a
                        ...
                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500 group-hover:bg-white dark:bg-slate-900 dark:text-slate-400 dark:group-hover:bg-slate-800">
                          <i class="fa-solid" [class]="'fa-solid ' + item.icon" aria-hidden="true"></i>
                        </span>
                        <span class="min-w-0">
              ...
              @if (filteredGroups().length === 0) {
                <div class="px-3 py-8 text-center text-sm text-slate-400">Không tìm thấy mục cài đặt phù hợp.</div>
              }
            </nav>
          </div>
        </aside>
