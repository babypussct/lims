...
export interface BuildStandardBackfillRecordsInput {
  requestId: string;
  logId: string;
...
export interface StandardBackfillRecords {
  usageLog: UsageLog;
 ...
 */
export function buildStandardBackfillRecords(
  input: BuildStandardBackfillRecordsInput
): StandardBackfillRecords {
  if (!input.requestId || !input.logId) throw new Error('Thiếu mã hồ sơ nhập bù.');
  if (!input.standard.id) throw new Error('Chuẩn không có mã định danh.');
  if (!input.employeeUid || !input.employeeName.trim()) throw new Error('Thiếu người sử dụng chuẩn.');
  if (!input.enteredByUid || !input.enteredByName.trim()) throw new Error('Thiếu người nhập bù.');
  ...
  if (!input.purpose.trim()) throw new Error('Mục đích sử dụng là bắt buộc.');
