...
@Component({
  ...
  template: `
    @if (confirmationService.state().isVisible) {
      <div
        class="fixed inset-0 z-[99] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm fade-in no-print"
        appModalA11y
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200">
          <div class="p-6">
            <div class="flex items-start gap-4">
              ...
              <div>
                <h3 id="confirmation-modal-title" class="font-bold text-slate-800 mb-1">Xác Nhận Hành Động</h3>
                <p id="confirmation-modal-message" class="text-sm text-slate-600 whitespace-pre-wrap">{{ confirmationService.state().message }}</p>
              </div>
            </div>
          </div>
          <div class="bg-slate-50 p-4 flex justify-end gap-3">
            <button type="button" (click)="confirmationService.onCancel()" class="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-bold text-sm transition">
              {{ confirmationService.state().cancelText }}
            </button>
            <button type="button" (click)="confirmationService.onConfirm()" class="px-4 py-2 text-white rounded-lg font-bold text-sm shadow-md transition"
